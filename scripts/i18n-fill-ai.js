...
const PROVIDERS = {
  ...
  openrouter: {
    ...
    headers: (key) => ({
      Authorization: `Bearer ${key}`,
      'content-type': 'application/json',
      'HTTP-Referer': 'https://mipham-quant.local',
      'X-Title': 'Mipham Quant i18n'
    }),
    buildBody: (model, system, user) => ({
      model,
      messages: [
...
4. Do NOT translate these proper nouns/terms: BTC, ETH, USDT, USDC, API, OKX, Binance, Bitget, Bybit, Gate, MEXC, HTX, KuCoin, Kraken, TradingView, RSI, MACD, EMA, SMA, VWAP, Bollinger, ATR, Mipham Quant, AI, JSON.
