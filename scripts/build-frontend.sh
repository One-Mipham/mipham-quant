...
#   - export MIPHAM_QUANT_VUE_SRC=/path/to/vue-repo && ./scripts/build-frontend.sh
#   - or manually: rsync -a --delete /path/to/vue-repo/dist/ frontend/dist/
#
# Usage:
#   MIPHAM_QUANT_VUE_SRC=~/path/to/private-vue-repo ./scripts/build-frontend.sh
#
# Prerequisites:
...
echo "============================================"
echo "  Mipham Quant — sync frontend dist"
echo "============================================"

if [ -z "${MIPHAM_QUANT_VUE_SRC:-}" ] || [ ! -d "$MIPHAM_QUANT_VUE_SRC" ]; then
  echo "ERROR: Set MIPHAM_QUANT_VUE_SRC to the root of your private Vue repository clone."
  echo "Example: export MIPHAM_QUANT_VUE_SRC=\$HOME/work/Mipham Quant-Vue"
  exit 1
fi

VUE_DIR="$(cd "$MIPHAM_QUANT_VUE_SRC" && pwd)"
