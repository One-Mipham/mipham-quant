# Mipham Quant i18n 工具

补齐 `MiphamAI-Quant-Vue-src/src/locales/lang/*.js` 多语言文件缺失 key 的工具链。
...
```bash
# 单个文件回滚
copy MiphamAI-Quant-Vue-src\src\locales\lang\ja-JP.js.bak MiphamAI-Quant-Vue-src\src\locales\lang\ja-JP.js

# 全部回滚
for %f in (MiphamAI-Quant-Vue-src\src\locales\lang\*.bak) do copy "%f" "%~dpnf"
```
...
```bash
# 删除所有 .bak
del MiphamAI-Quant-Vue-src\src\locales\lang\*.bak
# 删除缓存
