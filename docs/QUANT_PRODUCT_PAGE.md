...
> **让量化研究更智能** · Smarter Quantitative Research

---

## 产品概述

**Mipham Quant**（米旁量化）是[北京华安麦逄科技有限公司](https://onemipham.com)（One Mipham Corporation）旗下的 AI 量化交易旗舰平台。平台面向 A 股、港股通及全球多市场，提供从数据采集、策略开发、因子研究、回测验证到实盘交易的全链路量化研究工作流。

产品核心理念：**"数据主权 + AI 赋能"**——通过 Docker Compose 一键本地部署，所有行情数据、策略代码、交易记录完全存储于用户自有服务器，确保绝对的数据隐私；同时内置 LLM 大模型驱动的市场分析、策略生成与代码辅助能力，让个人投资者也能享受机构级的 AI 量化工具。
...
传统量化工具往往割裂——数据获取用一个工具，策略回测用另一个，实盘交易再换一个。Mipham Quant 将这些环节无缝整合到统一平台中：
...
所有代码和数据均在用户本地或自有服务器运行。我们**不收集、不存储、不上传**任何用户的策略代码、交易记录或账户凭据。一行 `./start.sh`，整个技术栈（PostgreSQL + Redis + Flask + Nginx）在本地启动。
...
| **加密货币** | Binance / OKX / Bybit / Coinbase 等 12+ 交易所 | 7×24 永续合约 |
...
| 版本 | 类型 | 状态 |
|------|------|------|
| v0.1.0 | MVP 公测版 | ✅ 已发布 (2026-06) |
| 桌面版 (Desktop) | macOS/Windows/Linux | 🔄 内测中 |
| 移动端 (Mobile) | iOS/Android | 📋 规划中 |
