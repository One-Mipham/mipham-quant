# Mipham Quant 云服务器部署指南

本文面向生产/准生产环境，使用云服务器 + Docker Compose 部署 Mipham Quant，并补充域名、HTTPS、反向代理与前后端分离说明。
...
推荐站点配置 `/etc/nginx/sites-available/mipham-quant.conf`：
...
```bash
sudo ln -s /etc/nginx/sites-available/mipham-quant.conf /etc/nginx/sites-enabled/mipham-quant.conf
sudo nginx -t
