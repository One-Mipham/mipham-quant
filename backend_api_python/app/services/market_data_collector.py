...
class MarketDataCollector:
    ...
    def _get_price(self, market: str, symbol: str) -> dict[str, Any] | None:
        """
    ...
    def _get_kline(self, market: str, symbol: str, timeframe: str, limit: int = 60) -> list[dict[str, Any]] | None:
        """
    ...
    def _ema_series_sma_seed(self, data: list[float], period: int) -> list[float | None]:
        """
    ...
    def _get_fundamental(self, market: str, symbol: str) -> dict[str, Any] | None:
        """获取基本面数据"""
        try:
            if market == "USStock":
    ...
    def _get_cn_hk_fundamental(self, market: str, symbol: str) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            if needs_financials or not has_statements:
                ...
                if td_stmts:
                    ...
                    logger.info(
                        "TwelveData statements for %s:%s: %d/%d growth keys filled",
                        market,
                        symbol,
                        filled_td,
                        len(_growth_keys),
            ...
            if needs_financials:
                try:
                    ...
                    logger.debug(
                        "AkShare CN/HK financial indicators failed %s:%s: %s",
                        market,
                        symbol,
                        e,
                    )
                ...
                if fin_data:
                    ...
                    logger.info(
                        "CN/HK AkShare financial indicators for %s:%s: %d/%d growth keys filled",
                        market,
                        symbol,
                        filled,
                        len(_growth_keys),
            ...
            if "financial_statements" not in result:
                try:
                    ...
                    if stmts:
                        ...
                        logger.debug(
                            "CN/HK financial statements (AkShare) for %s: %s",
                            symbol,
                            list(stmts.keys()),
                        )
                except Exception as e:
    ...
    def _get_us_fundamental(self, symbol: str) -> dict[str, Any] | None:
        """
    ...
    def _get_financial_statements(self, symbol: str) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            try:
                ...
                if balance_sheet is not None and not balance_sheet.empty:
                    # 获取最近4个季度
                    latest_quarters = (
                        balance_sheet.columns[:4] if len(balance_sheet.columns) >= 4 else balance_sheet.columns
                    )
                    statements["balance_sheet"] = {
                        "latest_date": str(latest_quarters[0]) if len(latest_quarters) > 0 else None,
                        "total_assets": float(balance_sheet.loc["Total Assets", latest_quarters[0]])
                        if "Total Assets" in balance_sheet.index and len(latest_quarters) > 0
                        else None,
                        "total_liabilities": float(balance_sheet.loc["Total Liab", latest_quarters[0]])
                        if "Total Liab" in balance_sheet.index and len(latest_quarters) > 0
                        else None,
                        "total_equity": float(balance_sheet.loc["Stockholders Equity", latest_quarters[0]])
                        if "Stockholders Equity" in balance_sheet.index and len(latest_quarters) > 0
                        else None,
                        "cash": float(balance_sheet.loc["Cash", latest_quarters[0]])
                        if "Cash" in balance_sheet.index and len(latest_quarters) > 0
                        else None,
                        "debt": float(balance_sheet.loc["Total Debt", latest_quarters[0]])
                        if "Total Debt" in balance_sheet.index and len(latest_quarters) > 0
                        else None,
                        "current_assets": float(balance_sheet.loc["Current Assets", latest_quarters[0]])
                        if "Current Assets" in balance_sheet.index and len(latest_quarters) > 0
                        else None,
                        "current_liabilities": float(balance_sheet.loc["Current Liabilities", latest_quarters[0]])
                        if "Current Liabilities" in balance_sheet.index and len(latest_quarters) > 0
                        else None,
                    }
            except Exception as e:
            ...
            try:
                ...
                if income_stmt is not None and not income_stmt.empty:
                    ...
                    statements["income_statement"] = {
                        "latest_date": str(latest_quarters[0]) if len(latest_quarters) > 0 else None,
                        "total_revenue": float(income_stmt.loc["Total Revenue", latest_quarters[0]])
                        if "Total Revenue" in income_stmt.index and len(latest_quarters) > 0
                        else None,
                        "gross_profit": float(income_stmt.loc["Gross Profit", latest_quarters[0]])
                        if "Gross Profit" in income_stmt.index and len(latest_quarters) > 0
                        else None,
                        "operating_income": float(income_stmt.loc["Operating Income", latest_quarters[0]])
                        if "Operating Income" in income_stmt.index and len(latest_quarters) > 0
                        else None,
                        "net_income": float(income_stmt.loc["Net Income", latest_quarters[0]])
                        if "Net Income" in income_stmt.index and len(latest_quarters) > 0
                        else None,
                        "eps": float(income_stmt.loc["Basic EPS", latest_quarters[0]])
                        if "Basic EPS" in income_stmt.index and len(latest_quarters) > 0
                        else None,
                    }
            except Exception as e:
            ...
            try:
                ...
                if cashflow is not None and not cashflow.empty:
                    ...
                    statements["cash_flow"] = {
                        "latest_date": str(latest_quarters[0]) if len(latest_quarters) > 0 else None,
                        "operating_cash_flow": float(cashflow.loc["Operating Cash Flow", latest_quarters[0]])
                        if "Operating Cash Flow" in cashflow.index and len(latest_quarters) > 0
                        else None,
                        "investing_cash_flow": float(cashflow.loc["Capital Expenditure", latest_quarters[0]])
                        if "Capital Expenditure" in cashflow.index and len(latest_quarters) > 0
                        else None,
                        "financing_cash_flow": float(cashflow.loc["Financing Cash Flow", latest_quarters[0]])
                        if "Financing Cash Flow" in cashflow.index and len(latest_quarters) > 0
                        else None,
                        "free_cash_flow": float(cashflow.loc["Free Cash Flow", latest_quarters[0]])
                        if "Free Cash Flow" in cashflow.index and len(latest_quarters) > 0
                        else None,
                    }
            except Exception as e:
    ...
    def _get_earnings_data(self, symbol: str) -> dict[str, Any] | None:
        """
        获取盈利报告数据（Earnings）
        ...
        def _pick_float(stmt: pd.DataFrame, row_names: tuple, col) -> float | None:
    ...
    def _get_crypto_info(self, symbol: str) -> dict[str, Any] | None:
        """加密货币信息 (固定描述为主)"""
        # 常见加密货币的描述
        crypto_info = {
            "BTC": {
    ...
    def _get_crypto_factors(
        self, symbol: str, price_data: dict[str, Any], kline_data: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """采集加密货币专属交易大数据因子。"""
    ...
    def _coinglass_get(self, path: str, params: dict[str, Any], ttl_sec: int = 120) -> dict[str, Any] | None:
    ...
    def _cryptoquant_get(self, path: str, params: dict[str, Any], ttl_sec: int = 300) -> dict[str, Any] | None:
    ...
    def _safe_num(self, value: Any, default: float | None = None) -> float | None:
    ...
    def _pick_number(self, payload: Any, *keys: str, default: float | None = None) -> float | None:
        if isinstance(payload, dict):
            for key in keys:
                if key in payload:
    ...
    def _get_crypto_market_structure(
        self, symbol: str, price_data: dict[str, Any], kline_data: list[dict[str, Any]]
    ) -> dict[str, Any]:
        out = {
            "volume_24h": None,
    ...
    def _get_crypto_derivatives_metrics(self, symbol: str) -> dict[str, Any]:
        ...
        result["funding_rate"] = self._pick_number(
            latest or payload, "oi_weighted_funding_rate", "funding_rate", "fundingRate"
        )
        ...
        if isinstance(cached, dict):
            ...
            for k, v in cached.items():
                if merged.get(k) is None and v is not None:
                    merged[k] = v
            if any(
                merged.get(k) is not None for k in ("funding_rate", "open_interest", "long_short_ratio")
            ) and not merged.get("source"):
    ...
    def _derive_flow_bias(self, exchange_netflow: float | None, stablecoin_netflow: float | None) -> str:
        score = 0
    ...
    def _derive_squeeze_risk(
        ...
        hot_long = (
            ...
            and funding_rate > 0.03
            and long_short_ratio is not None
            and long_short_ratio > 1.5
        ...
        hot_short = (
            ...
            and funding_rate < -0.03
            and long_short_ratio is not None
            and long_short_ratio < 0.75
            and oi_change is not None
            and oi_change > 8
        )
        if hot_long or hot_short:
            return "high"
        if (funding_rate is not None and abs(funding_rate) > 0.015) or (
            long_short_ratio is not None and (long_short_ratio > 1.3 or long_short_ratio < 0.85)
        ):
            return "medium"
        return "low"

    def _derive_volume_state(self, volume_change_24h: float | None) -> str:
    ...
    def _build_crypto_factor_summary(
        ...
        *,
        ...
        funding_rate: float | None,
        open_interest_change_24h: float | None,
        exchange_netflow: float | None,
        stablecoin_netflow: float | None,
        signals: dict[str, Any],
    ) -> str:
        parts: list[str] = []
        if open_interest_change_24h is not None:
            parts.append(
                f"OI {'上升' if open_interest_change_24h >= 0 else '回落'} {abs(open_interest_change_24h):.1f}%"
            )
        if funding_rate is not None:
        ...
        outlook = (
            "偏多"
            if direction == "bullish" or flow == "bullish"
            else ("偏空" if direction == "bearish" or flow == "bearish" else "中性")
        )
        risk_text = {
            "high": "拥挤风险高",
            "medium": "拥挤度抬升",
    ...
    def _get_company(self, market: str, symbol: str) -> dict[str, Any] | None:
        """获取公司信息"""
        try:
    ...
    def _get_cn_hk_company(self, market: str, symbol: str) -> dict[str, Any] | None:
        """
    ...
    def _get_news(self, market: str, symbol: str, company_name: str = None, timeout: int = 8) -> dict[str, Any]:
        """
        获取新闻和情绪数据
    ...
    def _get_news_from_search(self, market: str, symbol: str, company_name: str = None) -> list[dict[str, Any]]:
        """
        从搜索引擎获取新闻
        ...
        try:
            ...
            response = search_service.search_stock_news(
                stock_code=symbol, stock_name=search_name, market=market, max_results=5
            )
            ...
            for market_data in related_markets:
                ...
                if market_id and market_id not in seen:
                    ...
                    if not polymarket_url:
                        ...
                        if (
                            slug
                            and not str(slug).isdigit()
                            and ("-" in str(slug) or any(c.isalpha() for c in str(slug)))
                        ):
                            # 使用有效的 slug
    ...
    def _extract_polymarket_keywords(self, symbol: str, market: str) -> list[str]:
        ...
        base_symbol = symbol.split("/", maxsplit=1)[0] if "/" in symbol else symbol
        ...
        logger.info(
            f"Extracted {len(unique_keywords)} Polymarket keywords (optimized from {len(keywords)}): {unique_keywords}"
        )
