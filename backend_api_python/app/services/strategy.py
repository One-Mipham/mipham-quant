...
class StrategyService:
    ...
    def get_running_strategies(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to fetch running strategies: {e!s}")
            return []
        ...
        try:
            ...
            if ex in (
                ...
                if ex == "bybit":
                    ...
                    return {
                        "success": True,
                        "message": f"Success, {len(symbols)} trading pairs",
                        "symbols": symbols,
                    }
                    ...
                    if market_type == "spot":
                        ...
                        if isinstance(j, list):
                            for it in j:
                                if not isinstance(it, dict):
                                    continue
                                if str(it.get("trade_status") or "").lower() not in (
                                    "tradable",
                                    "trading",
                                    "",
                                ):
            ...
            if not exchange_class:
                return {
                    "success": False,
                    "message": f"Unsupported exchange: {exchange_id}",
                    "symbols": [],
                }
            ...
            logger.error(f"Failed to fetch symbols: {e!s}")
            return {
                "success": False,
                "message": f"Failed to get trading pairs: {e!s}",
                "symbols": [],
            }
    ...
    def test_exchange_connection(self, exchange_config: dict[str, Any], user_id: int = 1) -> dict[str, Any]:
        ...
        with StrategyService._connection_test_semaphore:
            try:
                ...
                from app.services.live_trading.coinbase_exchange import (
                    CoinbaseExchangeClient,
                )
                from app.services.live_trading.deepcoin import DeepcoinClient
                from app.services.live_trading.factory import create_client
                from app.services.live_trading.gate import (
                    GateSpotClient,
                ...
                def _probe_market_type(market_type: str):
                    try:
                        ...
                        return {
                            "success": False,
                            "message": f"Create client failed: {e!s}",
                            "data": {
                                "exchange": safe_cfg,
                ...
                logger.error(f"test_exchange_connection failed: {e!s}")
                return {
                    "success": False,
    ...
    def _build_bot_display(self, trading_config: dict[str, Any]) -> dict[str, Any]:
        ...
        if bot_type == "martingale":
            ...
            display["strategy_params"] = [
                self._display_item(
                    "initialAmount",
                    "trading-bot.martingale.initialAmountAuto",
                    first_order,
                    "usdt",
                ),
                self._display_item(
                    "multiplier",
                    "trading-bot.martingale.multiplier",
                    self._to_float(params.get("multiplier"), 2.0),
                    "number",
                ),
                self._display_item(
                    "maxLayers",
                    "trading-bot.martingale.maxLayers",
                    max_layers,
                    "number",
                ),
                self._display_item(
                    "priceDropPct",
                    "trading-bot.martingale.priceDropTrigger",
                    self._to_float(params.get("priceDropPct"), 0.0),
                    "percent",
                ),
                self._display_item(
                    "takeProfitPct",
                    "trading-bot.martingale.avgEntryTakeProfit",
                    self._to_float(params.get("takeProfitPct"), 0.0),
                    "percent",
                ),
                self._display_item(
                    "stopLossPct",
                    "trading-bot.martingale.avgEntryStopLoss",
                    self._to_float(params.get("stopLossPct"), 0.0),
                    "percent",
                ),
                self._display_item(
                    "direction",
                    "trading-bot.martingale.direction",
                    params.get("direction") or "long",
                    "enum",
                    f"trading-bot.martingale.{params.get('direction') or 'long'}",
                ),
            ]
            if self._to_float(tc.get("max_daily_loss"), 0.0) > 0:
                display["risk_params"].append(
                    self._display_item(
                        "maxDailyLoss",
                        "trading-bot.martingale.maxDailyLossAdvanced",
                        self._to_float(tc.get("max_daily_loss"), 0.0),
                        "usdt",
                    )
                )
            return display
        ...
        if bot_type == "grid":
            display["strategy_params"] = [
                self._display_item(
                    "upperPrice",
                    "trading-bot.grid.upperPrice",
                    self._to_float(params.get("upperPrice"), 0.0),
                    "usdt",
                ),
                self._display_item(
                    "lowerPrice",
                    "trading-bot.grid.lowerPrice",
                    self._to_float(params.get("lowerPrice"), 0.0),
                    "usdt",
                ),
                self._display_item(
                    "gridCount",
                    "trading-bot.grid.gridCount",
                    self._to_int(params.get("gridCount"), 0),
                    "number",
                ),
                self._display_item(
                    "amountPerGrid",
                    "trading-bot.grid.amountPerGrid",
                    self._to_float(params.get("amountPerGrid"), 0.0),
                    "usdt",
                ),
                self._display_item(
                    "gridMode",
                    "trading-bot.grid.mode",
                    params.get("gridMode") or "arithmetic",
                    "enum",
                    f"trading-bot.grid.{params.get('gridMode') or 'arithmetic'}",
                ),
                self._display_item(
                    "gridDirection",
                    "trading-bot.grid.direction",
                    params.get("gridDirection") or "neutral",
                    "enum",
                    f"trading-bot.grid.{params.get('gridDirection') or 'neutral'}",
                ),
                self._display_item(
                    "orderMode",
                    "trading-bot.grid.orderType",
                    params.get("orderMode") or "maker",
                    "enum",
                    "trading-bot.grid.limitOrder"
                    if (params.get("orderMode") or "maker") == "maker"
                    else "trading-bot.grid.marketOrder",
                ),
            ]
        elif bot_type == "trend":
            ...
            display["strategy_params"] = [
                self._display_item(
                    "maPeriod",
                    "trading-bot.trend.maPeriod",
                    self._to_int(params.get("maPeriod"), 0),
                    "number",
                ),
                self._display_item(
                    "maType",
                    "trading-bot.trend.maType",
                    params.get("maType") or "EMA",
                    "text",
                ),
                self._display_item(
                    "confirmBars",
                    "trading-bot.trend.confirmBars",
                    self._to_int(params.get("confirmBars"), 0),
                    "number",
                ),
                self._display_item(
                    "positionPct",
                    "trading-bot.trend.positionPct",
                    self._to_float(params.get("positionPct"), 0.0),
                    "percent",
                ),
                self._display_item(
                    "direction",
                    "trading-bot.trend.direction",
                    direction,
                    "enum",
                    direction_key,
                ),
            ...
            display["strategy_params"] = [
                self._display_item(
                    "amountEach",
                    "trading-bot.dca.amountEach",
                    self._to_float(params.get("amountEach"), 0.0),
                    "usdt",
                ),
                self._display_item(
                    "frequency",
                    "trading-bot.dca.frequency",
                    frequency,
                    "enum",
                    frequency_key,
                ),
                self._display_item(
                    "totalBudget",
                    "trading-bot.dca.totalBudget",
                    self._to_float(params.get("totalBudget"), 0.0),
                    "usdt",
                ),
                self._display_item(
                    "dipBuyEnabled",
                    "trading-bot.dca.dipBuy",
                    bool(params.get("dipBuyEnabled")),
                    "bool",
                ),
                self._display_item(
                    "dipThreshold",
                    "trading-bot.dca.dipThreshold",
                    self._to_float(params.get("dipThreshold"), 0.0),
                    "percent",
                ),
            ]
        ...
        if self._to_float(tc.get("stop_loss_pct"), 0.0) > 0:
            display["risk_params"].append(
                self._display_item(
                    "stopLossPct",
                    "trading-bot.risk.stopLossPct",
                    self._to_float(tc.get("stop_loss_pct"), 0.0),
                    "percent",
                )
            )
        if self._to_float(tc.get("take_profit_pct"), 0.0) > 0:
            display["risk_params"].append(
                self._display_item(
                    "takeProfitPct",
                    "trading-bot.risk.takeProfitPct",
                    self._to_float(tc.get("take_profit_pct"), 0.0),
                    "percent",
                )
            )
        if self._to_float(tc.get("max_position"), 0.0) > 0:
            display["risk_params"].append(
                self._display_item(
                    "maxPosition",
                    "trading-bot.risk.maxPosition",
                    self._to_float(tc.get("max_position"), 0.0),
                    "usdt",
                )
            )
        if self._to_float(tc.get("max_daily_loss"), 0.0) > 0:
    ...
    def get_strategy(self, strategy_id: int, user_id: int = None) -> dict[str, Any] | None:
        ...
        try:
            with get_db_connection() as db:
                ...
                if user_id is not None:
                    cur.execute(
                        "SELECT * FROM qd_strategies_trading WHERE id = ? AND user_id = ?",
                        (strategy_id, user_id),
                    )
                else:
                    cur.execute(
                        "SELECT * FROM qd_strategies_trading WHERE id = ?",
                        (strategy_id,),
                    )
                r = cur.fetchone()
                cur.close()
    ...
    def create_strategy(self, payload: dict[str, Any]) -> int:
        ...
        exchange_id = (
            (resolved_ex_cfg.get("exchange_id") or "").strip().lower() if isinstance(resolved_ex_cfg, dict) else ""
        )
    ...
    def batch_create_strategies(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        exchange_config = payload.get("exchange_config") or {}
        from app.services.exchange_execution import (
            resolve_exchange_config as _resolve_ex,
        )
    ...
    def update_strategy(self, strategy_id: int, payload: dict[str, Any], user_id: int = None) -> bool:
        ...
        if ex_id == "mt5" and market_category != "Forex":
            raise ValueError(f"MT5 can only be used for Forex trading, but market_category is '{market_category}'.")
        if ex_id == "ibkr" and market_category != "USStock":
            raise ValueError(f"IBKR can only be used for US stock trading, but market_category is '{market_category}'.")
        ...
        strategy_type = (
            payload.get("strategy_type") if payload.get("strategy_type") is not None else existing.get("strategy_type")
        ) or "IndicatorStrategy"
        strategy_mode = (
            payload.get("strategy_mode") if payload.get("strategy_mode") is not None else existing.get("strategy_mode")
        ) or "signal"
