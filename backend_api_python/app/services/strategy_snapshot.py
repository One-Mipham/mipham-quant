...
class StrategySnapshotResolver:
    ...
    def _fetch_indicator_code(self, indicator_id: int | None) -> str:
        if not indicator_id:
            return ""
        try:
    ...
    def resolve(self, strategy: dict[str, Any], override_config: dict[str, Any] | None = None) -> dict[str, Any]:
        if not strategy:
        ...
        initial_capital = self._to_float(
            override.get(
                "initialCapital",
                trading_config.get("initial_capital", strategy.get("initial_capital", 10000)),
            ),
            10000.0,
        )
        leverage = self._to_int(
            override.get("leverage", trading_config.get("leverage", strategy.get("leverage", 1))),
            1,
        )
