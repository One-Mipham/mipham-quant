...
class HtxClient(BaseRestClient):
    def __init__(
        ...
        *,
        ...
        self._account_type: int | None = None  # 1=non-unified, 2=unified
        self._account_type_ts: float = 0.0

    @staticmethod
    def _format_swap_client_order_id(client_order_id: str | None) -> int | None:
    ...
    def _format_spot_client_order_id(self, client_order_id: str | None) -> str:
    ...
    def _spot_public_request(self, method: str, path: str, *, params: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    def _swap_public_request(self, method: str, path: str, *, params: dict[str, Any] | None = None) -> dict[str, Any]:
        old_base = self.base_url
        self.base_url = self.futures_base_url
        try:
            code, data, text = self._request(method, path, params=params)
    ...
    def _detect_account_type(self) -> int:
        ...
        try:
            ...
            if v3_code is not None and int(v3_code) != 200:
                logger.warning(
                    "HTX swap_unified_account_type returned code=%s msg=%s",
                    v3_code,
                    raw.get("msg"),
                )
            ...
            self._account_type_ts = now
            logger.info(
                "HTX account_type=%s (%s)",
                self._account_type,
                "non-unified" if at == 1 else "unified" if at == 2 else "unknown",
            )
    ...
    def get_balance(self) -> Any:
            ...
            try:
                if tag == "v3-unified":
                    ...
                    if str(raw.get("status") or "").lower() == "error":
                        logger.warning(
                            "HTX %s returned error: err_code=%s msg=%s",
                            tag,
                            raw.get("err_code"),
                            raw.get("err_msg"),
                        )
                ...
                if data is not None:
                    ...
                    if tag == "v1-isolated" and isinstance(data, list):
                        has_bal = any(
                            float(it.get("margin_balance") or 0) > 0 or float(it.get("margin_available") or 0) > 0
                            for it in data
        ...
        try:
            ...
            if usdt_total > 0 or usdt_avail > 0:
                logger.info(
                    "HTX spot USDT fallback: avail=%.4f total=%.4f",
                    usdt_avail,
                    usdt_total,
                )
                return {
        ...
        logger.warning(
            "HTX all balance endpoints failed (acct_type=%s), last error: %s",
            acct,
            last_err,
        )
        return {"data": []}
    ...
    def get_positions(self, *, symbol: str = "") -> Any:
        ...
        if acct == 2:
            ...
            try:
                ...
                if data is not None:
                    logger.info(
                        "HTX positions succeeded via v1-isolated (items=%d)",
                        len(data) if isinstance(data, list) else -1,
                    )
                    return raw
            except Exception as e:
                logger.info(
                    "HTX unified account v1-isolated position query failed (expected): %s",
                    e,
                )
            return {"data": []}
        ...
        logger.warning(
            "HTX all position endpoints failed for symbol=%s, last error: %s",
            symbol,
            last_err,
        )
        return {"data": []}
    ...
    def get_ticker(self, *, symbol: str) -> dict[str, Any]:
        if self.market_type == "spot":
            raw = self._spot_public_request(
                ...
                params={"symbol": to_htx_spot_symbol(symbol)},
            )
        else:
            raw = self._swap_public_request(
                "GET",
                "/linear-swap-ex/market/detail/merged",
                params={"contract_code": to_htx_contract_code(symbol)},
            )
    ...
    def cancel_order(self, *, symbol: str, order_id: str = "", client_order_id: str = "") -> dict[str, Any]:
        if self.market_type == "spot":
            if order_id:
                return self._spot_private_request("POST", f"/v1/order/orders/{order_id!s}/submitcancel")
            if client_order_id:
    ...
    def get_order(self, *, symbol: str, order_id: str = "", client_order_id: str = "") -> dict[str, Any]:
        if self.market_type == "spot":
            if order_id:
                raw = self._spot_private_request("GET", f"/v1/order/orders/{order_id!s}")
