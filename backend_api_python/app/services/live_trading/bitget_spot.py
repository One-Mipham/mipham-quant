...
class BitgetSpotClient(BaseRestClient):
    ...
    def _normalize_base_size(self, *, symbol: str, base_size: float) -> tuple[Decimal, int | None]:
        """
        ...
        *,
        ...
        while True:
            ...
            if timed_out:
                ...
                try:
                    filled = float(
                        row.get("baseVolume") or row.get("filledQty") or row.get("dealSize") or row.get("size") or 0.0
                    )
                except Exception:
                    filled = 0.0
                try:
                    quote_amt = float(
                        row.get("quoteVolume") or row.get("filledTotalAmount") or row.get("dealFunds") or 0.0
                    )
                ...
                if fee <= 0 and isinstance(row, dict):
                    ...
                    if fd_fee > 0:
                        ...
                        if not fee_ccy and fd_ccy:
                            fee_ccy = fd_ccy
                        logger.debug(
                            "Bitget Spot timeout fallback fee via feeDetail: %.8f %s",
                            fee,
                            fee_ccy,
                        )
                return {
