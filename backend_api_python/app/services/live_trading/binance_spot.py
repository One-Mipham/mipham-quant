...
class BinanceSpotClient(BaseRestClient):
    ...
    def _signed_request(self, method: str, path: str, *, params: dict[str, Any]) -> dict[str, Any]:
        ...
        for attempt in range(2):
            ...
            if code >= 400:
                err = LiveTradingError(f"BinanceSpot HTTP {code}: {text[:500]}{self._hint_binance_spot_2015(text)}")
    ...
    def get_fee_rate(self, symbol: str, market_type: str = "spot") -> dict[str, float] | None:
    ...
    def _fetch_commission_for_order(
        ...
        for attempt in range(3):
            try:
                ...
                if total_fee > 0:
                    logger.debug(
                        "BinanceSpot fee via myTrades: %.8f %s (order=%s)",
                        total_fee,
                        fee_ccy,
                        oid,
                    )
            ...
            try:
                ...
                if rate_info:
                    ...
                    if taker_rate > 0:
                        ...
                        logger.info(
                            "BinanceSpot fee via tradeFee rate: %.8f USDT (rate=%.6f, order=%s)",
                            calc_fee,
                            taker_rate,
                            oid,
                        )
