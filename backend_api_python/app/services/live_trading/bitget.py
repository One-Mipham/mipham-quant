...
class BitgetMixClient(BaseRestClient):
    ...
    def get_contract(self, *, symbol: str, product_type: str = "USDT-FUTURES") -> dict[str, Any]:
        ...
        raw = self._public_request(
            ...
            params={"productType": pt, "symbol": sym},
        )
        data = raw.get("data") if isinstance(raw, dict) else None
    ...
    def _normalize_size(self, *, symbol: str, product_type: str, base_size: float) -> tuple[Decimal, int | None]:
        """
    ...
    def _normalize_price(self, *, symbol: str, product_type: str, price: float) -> tuple[Decimal, int | None]:
        """
        ...
        if step <= 0:
            step = self._to_dec(
                contract.get("priceStep") or contract.get("priceMultiplier") or contract.get("tickSize") or "0"
            )
    ...
    def get_fee_rate(self, symbol: str, market_type: str = "swap") -> dict[str, float] | None:
        ...
        try:
            raw = self._signed_request(
                ...
                params={"symbol": sym, "businessType": product_type},
            )
            data = raw.get("data") if isinstance(raw, dict) else None
            if isinstance(data, dict):
    ...
    def set_leverage(
        ...
        *,
        ...
        try:
            ...
            ok = isinstance(resp, dict) and str(resp.get("code") or "") in (
                "00000",
                "0",
                "",
            )
    ...
    def wait_for_fill(
        ...
        *,
        ...
        def _fee_from_order_detail_row(drow: dict[str, Any]) -> tuple[Decimal, str]:
            ...
            ccy = str(
                drow.get("feeCoin") or drow.get("feeCcy") or drow.get("fillFeeCoin") or drow.get("deductFeeCoin") or ""
            ).strip()
                    ...
                    for f in fill_list:
                        try:
                            ...
                            ccy = str(
                                f.get("feeCoin")
                                or f.get("feeCcy")
                                or f.get("fillFeeCoin")
                                or f.get("feeCurrency")
                                or ""
                            ).strip()
