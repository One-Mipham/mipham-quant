...
class BinanceFuturesClient(BaseRestClient):
    def __init__(
        ...
        *,
        ...
        self._dual_side_cache: tuple[float, bool] | None = None
    ...
    @staticmethod
    def _dec_str(d: Decimal, max_decimals: int = 18, strict_precision: int | None = None) -> str:
        """
    ...
    def _format_client_order_id(self, client_order_id: str | None) -> str:
    ...
    def _signed_request(self, method: str, path: str, *, params: dict[str, Any]) -> dict[str, Any]:
        ...
        last_err: LiveTradingError | None = None
    ...
    def get_symbol_filters(self, *, symbol: str) -> dict[str, Any]:
        ...
        if isinstance(symbols, list) and symbols:
            ...
            try:
                picked = next(
                    (s for s in symbols if isinstance(s, dict) and str(s.get("symbol") or "") == sym),
                    None,
                )
            except Exception:
                picked = None
    ...
    @staticmethod
    def _floor_to_precision(value: Decimal, precision: int | None) -> Decimal:
        try:
    ...
    def _normalize_quantity(self, *, symbol: str, quantity: float, for_market: bool) -> tuple[Decimal, int | None]:
        """
    ...
    def get_fee_rate(self, symbol: str, market_type: str = "swap") -> dict[str, float] | None:
    ...
    def get_dual_side_position(self) -> bool | None:
        """
    ...
    @staticmethod
    def _normalize_position_side(pos_side: str | None) -> str:
        ...
        *,
        ...
        while True:
            ...
            if filled > 0 and avg_price > 0:
                fee, fee_ccy = self._fetch_commission_for_order(
                    symbol=symbol, order_id=order_id, filled=filled, avg_price=avg_price
                )
                return {
                    "filled": filled,
                    "avg_price": avg_price,
    ...
    def _fetch_commission_for_order(
        self, *, symbol: str, order_id: str, filled: float, avg_price: float
    ) -> tuple[float, str]:
        ...
        for attempt in range(3):
            try:
                ...
                if total_fee > 0:
                    logger.debug(
                        "Binance fee via userTrades: %.8f %s (order=%s, attempt=%d)",
                        total_fee,
                        fee_ccy,
                        oid,
                        attempt,
            ...
            try:
                ...
                if rate_info:
                    ...
                    if taker_rate > 0:
                        ...
                        logger.info(
                            "Binance fee via commissionRate: %.8f USDT (rate=%.6f, order=%s)",
                            calc_fee,
                            taker_rate,
                            oid,
                        )
