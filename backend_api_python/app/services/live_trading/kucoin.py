...
class KucoinSpotClient(BaseRestClient):
    ...
    def ping(self) -> bool:
        try:
            ...
            return isinstance(d, dict) and str(d.get("code") or "") in (
                "200000",
                "0",
                "",
            )
    ...
    def get_ticker(self, *, symbol: str) -> dict[str, Any]:
        raw = self._public_request(
            ...
            params={"symbol": to_kucoin_symbol(symbol)},
        )
    ...
    def cancel_order(self, *, order_id: str = "", client_order_id: str = "") -> Any:
        if order_id:
            return self._signed_request("DELETE", f"/api/v1/orders/{order_id!s}")
        if client_order_id:
            return self._signed_request("DELETE", f"/api/v1/order/client-order/{client_order_id!s}")
    ...
    def get_order(self, *, order_id: str = "", client_order_id: str = "") -> Any:
        if order_id:
            return self._signed_request("GET", f"/api/v1/orders/{order_id!s}")
        if client_order_id:
            return self._signed_request("GET", f"/api/v1/order/client-order/{client_order_id!s}")
...
class KucoinFuturesClient(BaseRestClient):
    ...
    def cancel_order(self, *, order_id: str = "", client_order_id: str = "") -> Any:
        if order_id:
            return self._signed_request("DELETE", f"/api/v1/orders/{order_id!s}")
        if client_order_id:
            return self._signed_request("DELETE", f"/api/v1/orders/client-order/{client_order_id!s}")
