...
class _GateBase(BaseRestClient):
    ...
    def _format_text(self, client_order_id: str | None) -> str:
    ...
    def get_fee_rate(self, symbol: str, market_type: str = "swap") -> dict[str, float] | None:
        ...
        try:
            if market_type == "spot":
                ...
                settle = "usdt"
                contract = (
                    pair.replace("_", "").upper() + "_USDT"
                    if "_" not in pair.upper() or "USDT" not in pair.upper()
                    else pair
                )
...
class GateSpotClient(_GateBase):
    ...
    def cancel_order(self, *, order_id: str) -> Any:
        ...
        return self._signed_request("DELETE", f"/api/v4/spot/orders/{order_id!s}")

    def get_order(self, *, order_id: str) -> Any:
        if not order_id:
            raise LiveTradingError("Gate spot get_order requires order_id")
        return self._signed_request("GET", f"/api/v4/spot/orders/{order_id!s}")
...
class GateUsdtFuturesClient(_GateBase):
    ...
    def _resolve_order_size(self, *, contract: str, side: str, base_size: float) -> tuple[str, dict[str, str] | None]:
        """
        ...
        if dp > 0:
            ...
            if "." in s:
                s = s.rstrip("0").rstrip(".")
            return (
                s if s and s not in ("-", "+", "-0", "+0", "0") else "0",
    ...
    def cancel_order(self, *, order_id: str) -> Any:
        ...
        return self._signed_request("DELETE", f"/api/v4/futures/usdt/orders/{order_id!s}")

    def get_order(self, *, order_id: str) -> Any:
        if not order_id:
            raise LiveTradingError("Gate futures get_order requires order_id")
        return self._signed_request("GET", f"/api/v4/futures/usdt/orders/{order_id!s}")
