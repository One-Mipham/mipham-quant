...
class CommunityService:
    ...
    def get_indicator_detail(self, indicator_id: int, user_id: int = None) -> dict[str, Any] | None:
        """获取指标详情"""
        try:
            with get_db_connection() as db:
                cur = db.cursor()
    ...
    def _find_buyer_local_copy(
        self, cur, buyer_id: int, indicator_id: int, original_name: str = ""
    ) -> dict[str, Any] | None:
            ...
            with get_db_connection() as db:
                ...
                if (local.get("code") or "") == (original.get("code") or ""):
                    ...
                    return (
                        True,
                        "already_latest",
                        {"local_copy_id": local["id"], "updated": False},
                    )
    ...
    def admin_delete_indicator(self, admin_id: int, indicator_id: int) -> tuple[bool, str]:
        ...
        try:
            with get_db_connection() as db:
                ...
                # 删除关联的评论
                cur.execute(
                    "DELETE FROM qd_indicator_comments WHERE indicator_id = ?",
                    (indicator_id,),
                )

                # 删除关联的购买记录
                cur.execute(
                    "DELETE FROM qd_indicator_purchases WHERE indicator_id = ?",
                    (indicator_id,),
                )
