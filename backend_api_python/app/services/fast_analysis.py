...
def _safe_float_price(value: Any, default: float | None = None) -> float | None:
...
_GEO_SEVERE_PATTERNS: list[re.Pattern] = [
    ...
    re.compile(
        r"\b(?:military\s+attack|armed\s+attack|troops?\s+(?:fire|attack|invade))\b",
        re.I,
    ),
...
_GEO_CONTEXT_MODERATE: list[re.Pattern] = [
    ...
    re.compile(
        r"\b(?:middle\s+east|south\s+china\s+sea|taiwan\s+strait)\s+(?:crisis|tension|conflict)\b",
        re.I,
    ),
]
_GEO_ZH_SEVERE = (
...
def _geopolitical_match_level(combined_text: str) -> tuple[str, str | None]:
    """
...
"""
        ...
        system_prompt = f"""You are Mipham Quant's Senior Financial Analyst with 20+ years of experience. 
...
6. Provide your analysis now. Remember: all prices must be within 10% of ${current_price}."""
    ...
    def analyze(
        ...
        try:
            ...
            def _extract_current_price(d: dict[str, Any]) -> float | None:
            ...
            if len(ensemble_models) >= 2:
                ...
                for em in ensemble_models[:3]:
                    a = self.llm_service.safe_call_llm(
                        system_prompt,
                        user_prompt,
                        default_structure=default_struct,
                        model=em,
                    )
            ...
            if consensus_abs >= min_abs_override:
                ...
                analysis["confidence"] = int(
                    max(
                        ...
                        min(
                            100,
                            int(analysis.get("confidence", 50) or 50) * quality_multiplier,
                        ),
                    )
                )
            ...
            if market == "Crypto" and not detailed_analysis.get("fundamental"):
                detailed_analysis["fundamental"] = crypto_factor_summary or (data.get("crypto_factors") or {}).get(
                    "summary", ""
                )
    ...
    def _finalize_trading_plan_for_decision(
        self, analysis: dict, current_price: float, indicators: dict | None = None
    ) -> dict:
        """
    ...
    def _calculate_objective_score(self, data: dict[str, Any], current_price: float) -> dict[str, float]:
        ...
        fundamental_present = market_type in (
            "USStock",
            "CNStock",
            "HKStock",
        ) and _fundamental_meaningful(fundamental)
        ...
        return {
            ...
            "crypto_factor_summary": crypto_factor_objective.get("summary")
            or (crypto_factors.get("summary") if crypto_factors else ""),
        }
    ...
    def _calculate_crypto_factor_score(
        self, crypto_factors: dict[str, Any], price_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _store_analysis_memory(self, result: dict, user_id: int = None) -> int | None:
    ...
    def _save_analysis_task(self, result: dict, user_id: int = None) -> int | None:
        """
