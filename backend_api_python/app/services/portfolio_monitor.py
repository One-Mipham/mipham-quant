...
def _resolve_notification_delivery(user_id: int, notification_config: dict[str, Any] | None) -> dict[str, Any]:
    """
    ...
    try:
        ...
        for _i, pos in enumerate(positions):
            ...
            if key in seen_keys:
                continue
            seen_keys.add(key)
            base = dict(unique_analyses[pos_to_unique[_i]])
            ...
            float(base.get("current_price") or 0)
            combined_pnl = sum(
...
def _build_html_report(
    ...
    texts = {
        ...
        "subtitle": "由 Mipham Quant AI 快速分析引擎生成"
        if is_zh
        else "Generated by Mipham Quant Fast AI Analysis Engine",
...
def _build_telegram_report(
    ...
    lines.extend(
        [
            ...
            f"<i>{'由 Mipham Quant 多智能体系统生成' if is_zh else 'Generated by Mipham Quant Multi-Agent System'}</i>",
        ]
    )
...
def _build_batch_telegram_report(
    ...
    if is_zh:
        ...
        if held:
            header.append(
                f"• 持仓: {len(held)} 个 | 总成本: ${total_cost:,.2f} | 盈亏: {pnl_sign}${total_pnl:,.2f} ({pnl_sign}{total_pnl_pct:.1f}%)"
            )
        if watched:
                    ...
                    with get_db_connection() as db:
                        ...
                        cur.execute(
                            ...
                            (
                                ...
                                html_report,
                                json.dumps(
                                    {"batch": True, "count": len(successful)},
                                    ensure_ascii=False,
                                    default=str,
                                ),
                            ),
...
def _send_monitor_notification(
    ...
    try:
        ...
        if not result.get("success"):
            error_title = (
                f"⚠️ 资产监测失败: {monitor_name}" if language.startswith("zh") else f"⚠️ Monitor Failed: {monitor_name}"
            )
