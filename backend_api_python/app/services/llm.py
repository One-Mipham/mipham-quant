...
class LLMService:
    ...
    def _call_openai_compatible(
        ...
        if "openrouter" in base_url:
            headers["HTTP-Referer"] = "https://mipham.ai"
            headers["X-Title"] = "Mipham Quant Analysis"
        ...
        if response.status_code >= 400:
            ...
            if "openrouter" in (base_url or "").lower():
                ...
                if not APIKeys.OPENROUTER_API_KEY:
                    ...
                    error_msg += (
                        ". 可能原因：API 密钥无效/过期、余额不足、或无模型权限。请检查 https://openrouter.ai/keys"
                    )
                elif response.status_code == 404:
                    error_msg += (
                        ". 可能原因：模型不可用或账户隐私/数据策略限制。请检查 https://openrouter.ai/settings/privacy"
                    )
    ...
    def _detect_provider_from_model(self, model: str) -> LLMProvider | None:
        """
    ...
    def call_llm_api(
        ...
        use_json_mode: bool = True,
        try_alternative_providers: bool = True,
    ) -> str:
        """
        ...
        explicit_provider_name = (
            str(cfg.get("llm", {}).get("provider") or os.getenv("LLM_PROVIDER", "")).strip().lower()
        )
        ...
        if not api_key:
            ...
            if not api_key:
                raise ValueError(
                    f"API key not configured for provider: {p.value}. Please configure at least one LLM provider API key."
                )
        ...
        models_to_try = [model]
        PROVIDER_CONFIGS[p]["default_model"]
        if use_fallback:
        ...
        for current_model in models_to_try:
            try:
                ...
                if status_code in (402, 403) and try_alternative_providers and current_model == models_to_try[-1]:
                    ...
                    logger.warning(
                        f"{p.value} returned {status_code} (likely API key issue). Trying alternative providers..."
                    )
                ...
                logger.error(f"{p.value} API request error ({current_model}): {e!s}")
                last_error = str(e)
                if not use_fallback or current_model == models_to_try[-1]:
                    raise

            except ValueError as e:
                logger.warning(f"Model {current_model} returned invalid data: {e!s}")
                last_error = str(e)
    ...
    def _try_alternative_providers(
        ...
        temperature: float,
        use_json_mode: bool,
        excluded_provider: LLMProvider = None,
    ) -> str:
        """
        ...
        for alt_provider in priority_order:
            ...
            try:
                ...
                logger.warning(f"Alternative provider {alt_provider.value} also failed: {e!s}")
                continue

        raise Exception("All LLM providers failed. Please check your API key configurations.")
    ...
    def safe_call_llm(
        ...
        try:
            ...
            default_structure["report"] = (
                f"Failed to parse analysis result JSON. Raw output (partial): {response_text[:500] if response_text else 'N/A'}"
            )
            return default_structure
        except Exception as e:
            logger.error(f"LLM call failed: {e!s}")
            default_structure["report"] = f"Analysis failed: {e!s}"
            return default_structure
