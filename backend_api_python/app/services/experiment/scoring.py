...
class StrategyScoringService:
    ...
    def rank_results(self, items: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
        ranked = list(items)
        ranked.sort(
            key=lambda item: float(((item.get("score") or {}).get("overallScore")) or 0.0),
            reverse=True,
        )
    ...
    def _estimate_regime_fit(self, regime: dict[str, Any], components: dict[str, float]) -> float:
        ...
        if regime_key == "range_compression":
            return min(
                100.0,
                components["winRateScore"] * 0.6 + components["stabilityScore"] * 0.4,
            )
        if regime_key == "high_volatility":
            return min(
                100.0,
                components["drawdownScore"] * 0.6 + components["profitFactorScore"] * 0.4,
            )
