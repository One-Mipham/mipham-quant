...
class ExperimentRunnerService:
        ...
        *,
        ...
        for round_num in range(1, max_rounds + 1):
            ...
            if global_best_score >= early_stop:
                logger.info(
                    "Early stop: score %.1f >= %.1f at round %d",
                    global_best_score,
                    early_stop,
                    round_num,
                )
    ...
    def _apply_candidate_to_snapshot(
        ...
        if ind_params:
            snap["indicator_params"] = {
                **(snap.get("indicator_params") or {}),
                **ind_params,
            }
    ...
    @staticmethod
    def _emit(callback: Callable | None, event: str, data: dict[str, Any]) -> None:
        if callback:
            try:
        ...
        *,
        ...
        for candidate in candidates:
            try:
                ...
                logger.error(
                    "structured_tune backtest failed for %s: %s",
                    candidate.get("name"),
                    exc,
                )
                result = {}
