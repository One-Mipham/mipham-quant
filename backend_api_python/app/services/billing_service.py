...
class BillingService:
    ...
    def get_user_vip_status(self, user_id: int) -> tuple[bool, datetime | None]:
        """
        获取用户VIP状态
            ...
            with get_db_connection() as db:
                ...
                cur.execute(
                    ...
                    (
                        ...
                        float(diff),
                        amount,
                        remark or f"Admin adjust: {old_credits} -> {amount}",
                        operator_id,
                    ),
                )
                    ...
                    if d.get("created_at"):
                        ...
                        if hasattr(dt, "isoformat"):
                            if getattr(dt, "tzinfo", None) is not None:
                                d["created_at"] = dt.astimezone(UTC).strftime("%Y-%m-%dT%H:%M:%S") + "Z"
                            else:
