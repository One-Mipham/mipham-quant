...
class UsdtPaymentService:
    ...
    def _get_cfg(self) -> dict[str, Any]:
        return {
            ...
            "usdt_trc20_contract": (
                os.getenv("USDT_TRC20_CONTRACT", "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t") or ""
            ).strip(),
            ...
            "trongrid_page_limit": min(
                200,
                max(1, int(float(os.getenv("USDT_TRONGRID_PAGE_LIMIT", "200") or 200))),
            ),
    ...
    def _refresh_order_in_tx(self, cur, row: dict[str, Any]) -> None:
        ...
        if chain != "TRC20":
            logger.info(
                "USDT refresh skip order_id=%s reason=unsupported_chain chain=%s",
                order_id,
                chain,
            )
            return
        if status not in ("pending", "paid", "expired"):
            logger.info(
                "USDT refresh skip order_id=%s reason=bad_status status=%s",
                order_id,
                status,
            )
            return
        ...
        if (
            not tx
            and chain_note
            and (chain_note.startswith("trongrid_http=") or chain_note.startswith("trongrid_request_error:"))
        ):
            logger.warning(
        ...
        if tx:
            ...
            try:
                ...
                if tx_ts:
                    tx_time = datetime.fromtimestamp(int(tx_ts) / 1000.0, tz=UTC)
    ...
    def _confirm_and_activate_in_tx(self, cur, order_id: int, user_id: int, plan: str, tx_hash: str) -> None:
        ...
        try:
            ...
            logger.error(
                f"USDT activate membership failed: order={order_id} err={e}",
                exc_info=True,
            )

    def _find_trc20_usdt_incoming(
        self, address: str, amount_usdt: Decimal, created_at: Any | None
        ...
        try:
            for _ in range(max_pages):
                ...
                if resp.status_code != 200:
                    ...
                    return (
                        None,
                        f"trongrid_http={resp.status_code} body_head={body_head!r}",
                    )
            ...
            for row in rows:
                ...
                try:
                    with get_db_connection() as db:
                        cur = db.cursor()
                        cur.execute(
                            "SELECT status FROM qd_usdt_orders WHERE id = ?",
                            (order_id,),
                        )
    ...
    def _refresh_one_order_out_of_tx(self, row: dict[str, Any]) -> None:
        ...
        if status == "paid":
            ...
            ready = False
            if (paid_at and (now - paid_at).total_seconds() >= confirm_sec) or (not paid_at and confirm_sec <= 0):
                ready = True
            if ready:
