...
class SecurityService:
    ...
    def get_security_config(self) -> dict[str, Any]:
        ...
        mobile_url = (os.getenv("MOBILE_APP_DOWNLOAD_URL") or "").strip() or "https://mipham.ai/download/app.apk"
        return {
    ...
    def check_login_allowed(self, username: str, ip_address: str) -> tuple[bool, str]:
        ...
        if ip_blocked:
            ...
            return (
                False,
                f"Too many failed attempts from this IP. Try again in {minutes + 1} minutes.",
            )
        ...
        try:
            with get_db_connection() as db:
                ...
                if row and row["count"] > 0:
                    return (
                        False,
                        f"Please wait {self.code_rate_limit_seconds} seconds before requesting another code",
                    )
                ...
                if row and row["count"] >= self.code_ip_hourly_limit:
                    return (
                        False,
                        "Too many verification code requests from this IP. Try again later.",
                    )
