...
class OAuthService:
    ...
    def _oauth_state_ttl_minutes(self) -> int:
        try:
            return max(
                5,
                min(120, int(float(os.getenv("OAUTH_STATE_TTL_MINUTES", "20") or 20))),
            )
        except Exception:
            return 20
    ...
    def _ensure_oauth_state_schema(self, cur) -> None:
        ...
        try:
            ...
            cur.execute("CREATE INDEX IF NOT EXISTS idx_oauth_states_expires ON qd_oauth_states(expires_at)")
            OAuthService._state_schema_ensured = True
        except Exception as e:
            logger.debug(f"_ensure_oauth_state_schema: {e}")

    def _oauth_state_save(self, state: str, provider: str, redirect: str | None) -> None:
        # Store naive UTC for TIMESTAMP WITHOUT TIME ZONE columns.
        exp = (datetime.now(UTC) + timedelta(minutes=self._oauth_state_ttl_minutes())).replace(tzinfo=None)
    ...
    def get_or_create_user_from_oauth(self, oauth_info: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
        ...
        try:
            with get_db_connection() as db:
                ...
                cur.execute("UPDATE qd_users SET last_login_at = NOW() WHERE id = ?", (user_id,))
