...
class TradingExecutor:
    ...
    def __init__(self):
        ...
        self._exchange_fee_cache: dict[int, dict[str, float] | None] = {}
    ...
    def start_strategy(self, strategy_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to start strategy {strategy_id}: {e!s}")
    ...
    def stop_strategy(self, strategy_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to stop strategy {strategy_id}: {e!s}")
            logger.error(traceback.format_exc())
            return False
    ...
    def _script_orders_to_execution_signals(
        ...
        try:
            is_bot_script = bool(
                (trading_config or {}).get("bot_type") or (trading_config or {}).get("strategy_mode") == "bot"
            )
        except Exception:
            is_bot_script = False
        try:
        ...
        try:
            ...
            if is_script:
                ...
                if "\\n" in indicator_code and "\n" not in indicator_code:
                    try:
                        ...
                        logger.warning(
                            f"Strategy {strategy_id} JSON decode failed; falling back to manual unescape: {e!s}"
                        )
                        indicator_code = (
            ...
            while True:
                try:
                    ...
                    if current_time - last_kline_update_time >= kline_update_interval:
                        ...
                        # 3b. Indicator strategies: real-time recompute
                            ...
                            try:
                                ...
                                logger.warning(f"Strategy {strategy_id} realtime indicator recompute failed: {e!s}")
                    ...
                    logger.error(f"Strategy {strategy_id} loop error: {e!s}")
                    logger.error(traceback.format_exc())
            ...
            logger.error(f"Strategy {strategy_id} crashed: {e!s}")
            logger.error(traceback.format_exc())
    ...
    def _load_strategy(self, strategy_id: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            if strategy:
                ...
                if isinstance(exchange_config_str, str) and exchange_config_str:
                    try:
                        ...
                        logger.error(f"Strategy {strategy_id} failed to parse exchange_config: {e!s}")
                        # 尝试直接解析 JSON（向后兼容）
                        try:
                            strategy["exchange_config"] = json.loads(exchange_config_str)
                        except Exception:
            ...
            logger.error(f"Failed to load strategy config: {e!s}")
            return None
    ...
    def _query_exchange_fee_rate(
        ...
        try:
            ...
            if result:
                logger.info(
                    f"Strategy {strategy_id} exchange fee rate: maker={result['maker']}, taker={result['taker']}"
                )
            return result
        except Exception as e:
    ...
    def _fetch_latest_kline(
        ...
        try:
            ...
            logger.error(f"Failed to fetch K-lines for {market_category}:{symbol}: {e!s}")
            return []
                        ...
                        try:
                            with self._price_cache_lock:
                                self._price_cache[cache_key] = (
                                    float(price),
                                    time.time() + self._price_cache_ttl_sec,
                                )
                        except Exception:
                            pass
                    return price
    ...
    def _server_side_stop_loss_signal(
        ...
        try:
            ...
            logger.warning(f"Strategy {strategy_id} server-side stop-loss check failed: {e!s}")
            return None
    ...
    def _is_server_side_exit_enabled(self, trading_config: dict[str, Any] | None, config_key: str) -> bool:
        """
    ...
    def _update_dataframe_with_current_price(
        ...
        try:
            ...
            logger.error(f"Failed to update realtime candle: {e!s}")
            return df
    ...
    def _execute_indicator_with_prices(
        ...
        try:
            ...
            logger.error(f"Failed to execute indicator and extract prices: {e!s}")
            logger.error(traceback.format_exc())
            return None
    ...
    def _execute_indicator_df(
        ...
        try:
            ...
            logger.error(f"Failed to execute indicator script: {e!s}")
            logger.error(traceback.format_exc())
            return None, {}

    def _execute_indicator(self, indicator_code: str, df: pd.DataFrame, trading_config: dict[str, Any]) -> Any | None:
        """兼容旧版本"""
        ...
        try:
            with get_db_connection() as db:
                ...
                for pos in all_positions:
                    # 简化匹配逻辑：只匹配前缀
                    if pos["symbol"].split(":")[0] == symbol.split(":", maxsplit=1)[0]:
                        matched_positions.append(pos)

                cursor.close()
                return matched_positions
        except Exception as e:
            logger.error(f"Failed to fetch positions: {e!s}")
            return []
        ...
        try:
            ...
            if not self._is_signal_allowed(state, signal_type):
                append_strategy_log(
                    strategy_id,
                    "info",
                    f"Signal filtered by state machine: {signal_type} (state={state})",
                )
                return False
            ...
            if market_type == "spot" and "short" in signal_type:
                append_strategy_log(
                    strategy_id,
                    "info",
                    f"Signal rejected: spot market does not support {signal_type}",
                )
                return False
            ...
            if "close" in sig:
                ...
                if is_bot_script and position_size is not None and float(position_size) > 1.0 and current_price > 0:
                    usdt_notional = float(position_size)
                    close_qty = (
                        (usdt_notional * leverage) / current_price
                        if market_type != "spot"
                        else usdt_notional / current_price
                    )
            ...
            append_strategy_log(
                strategy_id,
                "error",
                f"Order enqueue failed: {signal_type} {symbol}, error={_err}",
            )
            return False
            ...
            append_strategy_log(
                strategy_id,
                "error",
                f"Signal execution exception: {signal_type} {symbol}, {e}",
            )
            return False
    ...
    def _is_entry_ai_filter_enabled(
        self,
        *,
        ai_model_config: dict[str, Any] | None,
            ...
            if billing.is_billing_enabled():
                ...
                try:
                    with get_db_connection() as db:
                        cur = db.cursor()
                        cur.execute(
                            "SELECT user_id FROM qd_strategies_trading WHERE id = ?",
                            (strategy_id,),
                        )
                        row = cur.fetchone()
                        cur.close()
    ...
    @staticmethod
    def _signal_reason_log_suffix(signal: dict[str, Any] | None) -> str:
    ...
    def _calculate_current_equity(
        ...
        for pos in positions:
            try:
                ...
                if current_price and normalized_symbol and pos_symbol.split(":", maxsplit=1)[0] == normalized_symbol:
    ...
    def _get_indicator_code_from_db(self, indicator_id: int) -> str | None:
        try:
            with get_db_connection() as db:
                cursor = db.cursor()
