...
class AnalysisMemory:
    ...
    def store(self, analysis_result: dict[str, Any], user_id: int = None) -> int | None:
        """
    ...
    def get_all_history(self, user_id: int = None, page: int = 1, page_size: int = 20) -> dict:
        ...
        try:
            ...
            with get_db_connection() as db:
                ...
                # Get total count
                cur.execute(
                    f"SELECT COUNT(*) as cnt FROM qd_analysis_memory {where_clause}",
                    params_count,
                )
        ...
        try:
            with get_db_connection() as db:
                ...
                if user_id:
                    ...
                    cur.execute(
                        "DELETE FROM qd_analysis_memory WHERE id = %s AND user_id = %s",
                        (memory_id, user_id),
                    )
                else:
            ...
            with get_db_connection() as db:
                ...
                """,
                    (
                        ...
                        f"Analysis failed: {error_message or ''!s}",
                        int(memory_id),
                    ),
                )
                ok = cur.rowcount > 0
            ...
            with get_db_connection() as db:
                ...
                for row in rows:
                    try:
                        ...
                        if (
                            (decision == "BUY" and return_pct > 2)
                            or (decision == "SELL" and return_pct < -2)
                            or (decision == "HOLD" and abs(return_pct) <= 5)
                        ):  # 2% threshold
                            was_correct = True
            ...
            with get_db_connection() as db:
                ...
                for row in rows:
                    try:
                        ...
                        logger.warning(
                            f"Failed to validate memory {row.get('id')}: {e}",
                            exc_info=True,
                        )
                        stats["errors"] += 1
