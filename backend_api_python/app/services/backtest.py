...
class _KlineCache:
    ...
    def put(self, key: str, df: pd.DataFrame, timeframe: str):
        ...
        with self._lock:
            ...
            self._store[key] = {"df": df.copy(), "expires": _time.time() + ttl}
...
class BacktestService:
    ...
    def __init__(self):
        ...
        # Feature flag: use Polars for equity curve + metrics computation
        # Defaults to 'polars' when polars package is installed, falls back to 'pandas'
        _default = "polars" if _HAS_POLARS else "pandas"
        self._use_polars = os.environ.get("BACKTEST_ENGINE", _default) == "polars"
    ...
    def get_run(self, *, user_id: int, run_id: int) -> dict[str, Any] | None:
        self.ensure_storage_schema()
        with get_db_connection() as db:
            cur = db.cursor()
            cur.execute(
    ...
    def run_multi_timeframe(
        ...
        if skip_mtf:
            ...
            if fallback_reason:
                ...
                if fallback_reason == "scale_rules_not_supported_in_mtf":
                    result["precision_info"]["message"] = (
                        "Using standard backtest because scale rules are not fully supported in MTF mode"
                    )
                elif fallback_reason == "signal_timing_not_supported_in_mtf":
                    result["precision_info"]["message"] = (
                        "Using standard backtest because this execution timing is not fully supported in MTF mode"
                    )
        ...
        logger.info(
            f"Multi-timeframe backtest: strategy_tf={timeframe}, exec_tf={exec_tf}, range={start_date} ~ {end_date}"
        )
        ...
        try:
            ...
            logger.error(f"MTF simulation failed: {e!s}")
        ...
        try:
            logger.info(
                f"Calculating metrics: equity_curve_len={len(equity_curve)}, trades_len={len(trades)}, initial_capital={initial_capital}"
            )
            ...
            logger.error(f"Failed to calculate metrics: {e!s}")
            logger.error(traceback.format_exc())
            raise
        ...
        try:
            ...
            logger.error(f"Failed to format result: {e!s}")
            logger.error(traceback.format_exc())
            raise
    ...
    def _simulate_trading_mtf(
        ...
        try:
            logger.info(
                f"Entering _simulate_trading_mtf: df_signal={len(df_signal)}, df_exec={len(df_exec)}, signals_type={type(signals)}"
            )
        except Exception as e:
        ...
        logger.info(
            f"Trading params: capital={capital}, leverage={lev}, entry_pct={entry_pct_cfg}, strategy_config={cfg}"
        )
            ...
            if hasattr(signals[first_key], "index"):
                ...
                if len(sig_index) > 0 and len(df_index) > 0:
                    ...
                    if not sig_index.equals(df_index):
                        logger.warning(
                            "Signal index does NOT match df_signal index! This may cause signal lookup failures."
                        )

        # Check if trade_direction is 'both' mode
        _is_both = str(trade_direction or "both").lower() == "both"
        ...
        if all(k in signals for k in ["open_long", "close_long", "open_short", "close_short"]):
            ...
            if not buy_series.index.equals(df_signal.index):
                logger.warning(
                    f"Buy signal index mismatch! Signal index: {buy_series.index[:5].tolist()}, df_signal index: {df_signal.index[:5].tolist()}"
                )
                buy_series = buy_series.reindex(df_signal.index, fill_value=False)
            if not sell_series.index.equals(df_signal.index):
                logger.warning(
                    f"Sell signal index mismatch! Signal index: {sell_series.index[:5].tolist()}, df_signal index: {df_signal.index[:5].tolist()}"
                )
        ...
        logger.info(
            f"Signal timeframe: {signal_timeframe} ({signal_tf_seconds}s), Exec timeframe: {exec_timeframe} ({exec_tf_seconds}s)"
        )
        ...
        for sig_time in df_signal.index:
            ...
            try:
                ...
                logger.warning(
                    f"Error accessing signal at {sig_time}: {e}, signal index: {norm_signals['open_long'].index[:5].tolist()}, df_signal index: {df_signal.index[:5].tolist()}"
                )
                continue
            except Exception as e:
        ...
        if signal_queue:
            ...
            logger.error(
                "Signal queue is empty! Backtest will fail. Check indicator code to ensure it generates buy/sell signals."
            )
        ...
        if len(df_exec) > 0:
            ...
            logger.info(
                f"First exec candle: open={first_row['open']}, high={first_row['high']}, low={first_row['low']}, close={first_row['close']}"
            )
        ...
        logger.info(
            f"Starting execution loop: {total_exec_candles} candles to process, {len(signal_queue)} signals in queue"
        )
        ...
        for i, (timestamp, row) in enumerate(df_exec.iterrows()):
            ...
            if i > 0 and i % progress_log_interval == 0:
                ...
                logger.info(
                    f"Execution progress: {i}/{total_exec_candles} ({progress_pct:.1f}%), trades={executed_trades_count}, position={position}"
                )
            # 爆仓后直接停止回测，输出结果
            if is_liquidated:
                break
            ...
            while signal_queue_idx < len(signal_queue):
                ...
                if i < 10 and signal_queue_idx < len(signal_queue):
                    logger.debug(
                        f"[i={i}] Checking signal #{signal_queue_idx}: {sig_type} @ {sig_effective_time}, exec_time={timestamp}, position={position}"
                    )
            ...
            for path_price in price_path:
                ...
                if pending_signal and path_price == open_:
                    ...
                    if executed_trades_count < 10:
                        logger.info(
                            f"Executing pending signal: {pending_signal} @ {timestamp}, path_price={path_price}, open={open_}, position={position}"
                        )
                    ...
                    if pending_signal == "open_long" and (position == 0 or (both_mode_active and position < 0)):
                        ...
                        if both_mode_active and position < 0:
                            ...
                            if executed_trades_count <= 10:
                                logger.info(
                                    f"Trade #{executed_trades_count}: close_short (before open_long) @ {timestamp}, price={close_price:.4f}, profit={close_profit:.2f}"
                                )
                            # 检查是否爆仓
                        ...
                        if executed_trades_count <= 10:
                            logger.info(
                                f"Trade #{executed_trades_count}: open_long @ {timestamp}, price={exec_price:.4f}, shares={shares:.4f}"
                            )
                        pending_signal = None
                    ...
                    # open_short: In both mode, first close long if any, then open short
                        ...
                        if both_mode_active and position > 0:
                            ...
                            if executed_trades_count <= 10:
                                logger.info(
                                    f"Trade #{executed_trades_count}: close_long (before open_short) @ {timestamp}, price={close_price:.4f}, profit={close_profit:.2f}"
                                )
                            # 检查是否爆仓
                            if capital < min_capital_to_trade:
                                is_liquidated = True
                        ...
                        if executed_trades_count <= 10:
                            logger.info(
                                f"Trade #{executed_trades_count}: open_short @ {timestamp}, price={exec_price:.4f}, shares={shares:.4f}"
                            )
                        pending_signal = None
        ...
        # Summary log
        logger.info(
            f"MTF simulation complete: executed_trades={executed_trades_count}, total_trades_recorded={len(trades)}, final_capital={capital:.2f}, final_position={position}"
        )
        if len(trades) == 0:
            if len(signal_queue) == 0:
                logger.error("No trades executed because signal queue is empty! This usually means:")
                ...
                raise ValueError(
                    "No signals generated by indicator code. Please check your indicator code to ensure it sets df['buy'] and/or df['sell'] columns with boolean values."
                )
            else:
                logger.error(
                    f"No trades executed despite {len(signal_queue)} signals in queue. signal_queue_idx={signal_queue_idx}"
                )
                ...
                raise ValueError(
                    f"No trades executed despite {len(signal_queue)} signals. Check signal timing and position state logic."
                )
    ...
    def run_strategy_snapshot(
        ...
        if bool(snapshot.get("enable_mtf")) and str(market).lower() in [
            "crypto",
            "cryptocurrency",
        ]:
    ...
    def _run_script_strategy(
        ...
        *,
        ...
        signals = self._execute_script_strategy(
            code,
            df,
            {
                "initial_capital": initial_capital,
    ...
    def run_code_strategy(self, code: str, symbol: str, timeframe: str, limit: int = 1000) -> dict[str, Any]:
        """
    ...
    def run(
        ...
        # 3-5. Simulate trading + metrics + format (Polars or pandas path)
        if self._use_polars and _HAS_POLARS:
            try:
                ...
                logger.warning(
                    f"Polars backtest engine failed ({e}), falling back to pandas. "
                    "Set BACKTEST_ENGINE=pandas to suppress this warning."
                )
        ...
        result["executionAssumptions"]["engineVersion"] = (
            "polars-v1" if (self._use_polars and _HAS_POLARS) else self.ENGINE_VERSION
        )
        self._attach_actual_range_to_result(result, df)
        return result

    def _compute_equity_curve_legacy(
        self,
        df: pd.DataFrame,
        signals,
        initial_capital: float,
        ...
        """Legacy pandas-based equity curve + metrics computation (full feature set)."""
        equity_curve, trades, total_commission = self._simulate_trading(
            df,
            signals,
            initial_capital,
    ...
    def _fetch_kline_data(
        ...
        if not kline_data:
            logger.warning(
                f"No candle data retrieved for {market}:{symbol}, timeframe={timeframe}, limit={limit}, before_time={before_time}"
            )
            return pd.DataFrame()
        ...
        try:
            ...
            if df.empty:
                logger.warning("DataFrame is empty after setting time index")
                return pd.DataFrame()
            ...
            if df_filtered.empty:
                ...
                if len(df) > 0:
                    ...
                    logger.debug(
                        f"[Backtest] 过滤后无K线 {market}:{symbol} {timeframe} upstream={data_start}~{data_end}"
                    )
                    return pd.DataFrame()
            ...
            logger.error(f"Error processing kline data: {e!s}")
            logger.error(traceback.format_exc())
            return pd.DataFrame()
        ...
        try:
            ...
            if all(col in executed_df.columns for col in ["open_long", "close_long", "open_short", "close_short"]):
                ...
                if not buy_series.index.equals(df.index):
                    logger.warning("Buy signal index mismatch in _execute_indicator! Reindexing...")
                    buy_series = buy_series.reindex(df.index, fill_value=False)
                if not sell_series.index.equals(df.index):
                    logger.warning("Sell signal index mismatch in _execute_indicator! Reindexing...")
                ...
                logger.info(
                    f"Indicator execution: buy signals={buy_count}, sell signals={sell_count}, total_candles={len(df)}"
                )

                signals = {"buy": buy_series, "sell": sell_series}
    ...
    def _execute_script_strategy(
        self, code: str, df: pd.DataFrame, runtime: dict[str, Any] | None = None
    ) -> dict[str, pd.Series]:
        runtime = runtime or {}
    ...
    def _simulate_trading_new_format(
        ...
        for i, (timestamp, row) in enumerate(df.iterrows()):
            ...
            if position != 0 and position_type in ["long", "short"]:
                ...
                if position_type == "long" and position > 0:
                    ...
                    if candidates:
                        ...
                        last_trend_add_anchor = last_dca_add_anchor = last_trend_reduce_anchor = (
                            last_adverse_reduce_anchor
                        ) = None
                ...
                if position_type == "short" and position < 0:
                    ...
                    if candidates:
                        ...
                        pri = {
                            "close_short_stop": 0,
                            "close_short_trailing": 1,
                            "close_short_profit": 2,
                        }
                        trade_type, trigger_price = sorted(candidates, key=lambda x: (pri.get(x[0], 99), -x[1]))[0]
                        exec_price_close = trigger_price * (1 + slippage)
            ...
            if position > 0 and close_long_arr[i]:
                ...
                lowest_since_entry = None
                trend_add_times = dca_add_times = trend_reduce_times = adverse_reduce_times = 0
                last_trend_add_anchor = last_dca_add_anchor = last_trend_reduce_anchor = last_adverse_reduce_anchor = (
                    None
                )
                ...
                if capital + profit <= 0:
                    logger.warning("Insufficient funds when closing short - liquidation")
                    liquidation_loss = self._liquidation_loss(capital)
                    capital = 0
                    is_liquidated = True
                    trades.append(
            ...
            if (
                (not main_signal_on_bar)
                and position != 0
                and position_type in ["long", "short"]
                and capital >= min_capital_to_trade
            ):
                # Long
                if position_type == "long" and position > 0:
                    ...
                    if (
                        trend_add_enabled
                        and trend_add_step_pct_eff > 0
                        and trend_add_size_pct > 0
                        and (trend_add_max_times == 0 or trend_add_times < trend_add_max_times)
                    ):
                    ...
                    if (
                        dca_add_enabled
                        and dca_add_step_pct_eff > 0
                        and dca_add_size_pct > 0
                        and (dca_add_max_times == 0 or dca_add_times < dca_add_max_times)
                    ):
                    ...
                    if (
                        trend_reduce_enabled
                        and trend_reduce_step_pct_eff > 0
                        and trend_reduce_size_pct > 0
                        and (trend_reduce_max_times == 0 or trend_reduce_times < trend_reduce_max_times)
                    ):
                    ...
                    if (
                        position_type == "long"
                        and position > 0
                        and adverse_reduce_enabled
                        and adverse_reduce_step_pct_eff > 0
                        and adverse_reduce_size_pct > 0
                        and (adverse_reduce_max_times == 0 or adverse_reduce_times < adverse_reduce_max_times)
                    ):
            ...
            if (
                open_long_arr[i]
                and (position == 0 or (both_mode_active and position < 0))
                and capital >= min_capital_to_trade
            ):
            ...
            if position != 0 and not is_liquidated:
                if position_type == "long" and low <= liquidation_price:
                    ...
                    if has_stop_loss and stop_loss_price > liquidation_price:
                        ...
                        logger.warning(
                            f"Long liquidation! entry={entry_price:.2f}, low={low:.2f}, "
                    ...
                    logger.warning(
                        f"[candle {i}] Short hit liquidation! entry={entry_price:.2f}, high={high:.2f}, liq_price={liquidation_price:.2f}, "
                    ...
                    if has_stop_loss and stop_loss_price < liquidation_price:
                        ...
                        # SL not strict enough, liquidation triggered
                        logger.warning(
                            f"Short liquidation! entry={entry_price:.2f}, high={high:.2f}, "
                            f"爆仓线={liquidation_price:.2f}, 止损价={stop_loss_price:.2f}"
                        )
                        is_liquidated = True
                        liquidation_loss = self._liquidation_loss(capital)
        ...
        if position != 0:
            ...
            if position > 0:  # Close long
                ...
                if capital + profit <= 0:
                    logger.warning("Liquidation at backtest end!")
                    liquidation_loss = self._liquidation_loss(capital)
                    capital = 0
                    is_liquidated = True
                    trades.append(
    ...
    def _simulate_trading_old_format(
        ...
        for i, (timestamp, row) in enumerate(df.iterrows()):
                        ...
                        trades.append(
                            {
                                "time": timestamp.strftime("%Y-%m-%d %H:%M"),
                                "type": {
                                    "stop": "close_long_stop",
                                    "profit": "close_long_profit",
                                    "trailing": "close_long_trailing",
                                }.get(reason, "close_long"),
                                "price": round(exec_price, 4),
                                "amount": round(position, 4),
                                "profit": round(profit, 2),
                                "balance": round(max(0, capital), 2),
                        ...
                        trades.append(
                            {
                                "time": timestamp.strftime("%Y-%m-%d %H:%M"),
                                "type": {
                                    "stop": "close_short_stop",
                                    "profit": "close_short_profit",
                                    "trailing": "close_short_trailing",
                                }.get(reason, "close_short"),
                                "price": round(exec_price, 4),
                                "amount": round(shares, 4),
                                "profit": round(profit, 2),
                                "balance": round(max(0, capital), 2),
            ...
            if trade_direction == "long":
                ...
                if signal == -1 and position == 0 and capital >= min_capital_to_trade:  # Sell to open short
                    logger.debug(
                        f"[Short mode] Sell to open short: time={timestamp}, price={price}, leverage={leverage}x"
                    )
                    base_price = open_ if signal_timing in ["next_bar_open", "next_open", "nextopen", "next"] else price
                    exec_price = base_price * (1 - slippage)
                    # With leverage: position = capital * leverage / price
                    ...
                    if capital + profit <= 0:
                        logger.warning(
                            f"Insufficient funds when closing short - liquidation: capital={capital:.2f}, loss={-profit:.2f}"
                        )
                        liquidation_loss = self._liquidation_loss(capital)
                        capital = 0
                        is_liquidated = True
                ...
                if signal == 1 and position == 0 and capital >= min_capital_to_trade:  # Buy to open long
                    ...
                    logger.debug(
                        f"[Both mode] Sell to open short: time={timestamp}, price={price}, leverage={leverage}x"
                    )
                    base_price = open_ if signal_timing in ["next_bar_open", "next_open", "nextopen", "next"] else price
                    exec_price = base_price * (1 - slippage)
                    # With leverage: position = capital * leverage / price
            ...
            if position != 0 and not is_liquidated:
                if position_type == "long":
                    ...
                    if price <= liquidation_price:
                        logger.warning(
                            f"Long liquidation! entry={entry_price:.2f}, current={price:.2f}, liq_price={liquidation_price:.2f}"
                        )
                        is_liquidated = True
                        liquidation_loss = self._liquidation_loss(capital)
                        capital = 0
                    ...
                    if price >= liquidation_price:
                        logger.warning(
                            f"Short liquidation! entry={entry_price:.2f}, current={price:.2f}, liq_price={liquidation_price:.2f}"
                        )
                        is_liquidated = True
                        liquidation_loss = self._liquidation_loss(capital)
                        capital = 0
        ...
        if position != 0:
            ...
            if position > 0:  # Close long
                ...
                if capital + profit <= 0:
                    logger.warning(
                        f"Liquidation at backtest end! Close short loss too large: capital={capital:.2f}, loss={-profit:.2f}"
                    )
                    is_liquidated = True
                    liquidation_loss = self._liquidation_loss(capital)
                    trades.append(
    ...
    def _execution_assumptions(
        ...
        *,
        ...
        mtf_fallback_reason: str | None = None,
    ) -> dict[str, Any]:
        """
    ...
    def _format_result(self, metrics: dict, equity_curve: list, trades: list) -> dict[str, Any]:
        """格式化回测结果"""
        ...
        for item in equity_curve:
            cleaned_curve.append({"time": item["time"], "value": clean_value(item["value"])})
...
# ---------------------------------------------------------------------------
# Polars-accelerated equity curve + metrics computation
# ---------------------------------------------------------------------------
...
def compute_equity_curve_polars(
    ...
    """
    Compute equity curve and backtest metrics using Polars (5-10x faster hot path).

    Args:
        df: K-line DataFrame with DatetimeIndex and OHLCV columns.
        signals: Signal dict from _execute_indicator (buy/sell or 4-way signals).
        initial_capital: Starting capital.
        commission: Commission rate (e.g. 0.0003 for 0.03%).
        slippage_pct: Slippage percentage (e.g. 0.001 for 0.1%).
        trade_direction: 'long', 'short', or 'both'.
        timeframe: K-line timeframe ('1D', '1H', etc.).

    Returns:
        Result dict compatible with BacktestService._format_result output format.
    """
    if not _HAS_POLARS:
        raise ImportError("polars is required for the Polars backtest engine")
    ...
    # --- Extract signals from the signal dict ---
    # Support both 4-way (open_long/close_long/open_short/close_short)
    # and 2-way (buy/sell) signal formats.
    if all(k in signals for k in ("open_long", "close_long", "open_short", "close_short")):
        if td == "long":
            buy_signal_raw = signals["open_long"]
            sell_signal_raw = signals["close_long"]
        elif td == "short":
            buy_signal_raw = signals["close_short"]
            sell_signal_raw = signals["open_short"]
        else:  # both
            buy_signal_raw = signals["open_long"]
            sell_signal_raw = signals["close_long"]
    elif all(k in signals for k in ("buy", "sell")):
        if td == "long":
            buy_signal_raw = signals["buy"]
            sell_signal_raw = signals["sell"]
        elif td == "short":
            buy_signal_raw = signals["sell"]
            sell_signal_raw = signals["buy"]
        else:  # both
            buy_signal_raw = signals["buy"]
            sell_signal_raw = signals["sell"]
    else:
        buy_signal_raw = signals.get("buy", pd.Series([False] * len(df)))
        sell_signal_raw = signals.get("sell", pd.Series([False] * len(df)))

    # Convert signal Series to numpy bool arrays
    def _series_to_bool(s) -> np.ndarray:
        if s is None or (hasattr(s, "empty") and s.empty):
            return np.zeros(len(df), dtype=bool)
        try:
            return s.fillna(False).astype(bool).to_numpy(dtype=np.bool_)
        except Exception:
            return np.zeros(len(df), dtype=bool)

    buy_signal = _series_to_bool(buy_signal_raw)
    sell_signal = _series_to_bool(sell_signal_raw)

    # The DataFrame has 'time' as DatetimeIndex, not a column.
    # Work with a reset-index copy so 'time' is a regular column.
    df_work = df.reset_index()  # brings 'time' back as a column
    if "time" not in df_work.columns:
        # Fallback: use the index values directly
        df_work["time"] = df.index.astype("int64") // 10**9  # datetime → unix seconds

    # Convert to Polars for vectorized operations.
    # Cast to numpy-native dtypes to avoid pyarrow dependency for nullable pandas types.
    _cols = df_work[["time", "open", "high", "low", "close"]].reset_index(drop=True)
    _cols = _cols.astype(
        {
            "open": "float64",
            "high": "float64",
    ...
    # time column: ensure int64 (Unix seconds) for consistent handling.
    # Normalize to datetime64[ns] first, then convert to seconds.
    if _cols["time"].dtype == "object":
        _cols["time"] = pd.to_datetime(_cols["time"])
    if pd.api.types.is_datetime64_any_dtype(_cols["time"]):
        _cols["time"] = _cols["time"].astype("datetime64[ns]").astype("int64") // 10**9
    _cols["time"] = _cols["time"].astype("int64")
    pdf = pl.from_pandas(_cols)
    n = len(pdf)

    closes = pdf["close"].to_numpy()
    pdf["open"].to_numpy()
    times = pdf["time"].to_numpy()

    # --- Sequential trade simulation (inherently stateful, keeps Python loop) ---
    position = 0.0
    cash = float(initial_capital)
    equity = np.zeros(n, dtype=np.float64)
    trades: list[dict[str, Any]] = []
    total_commission_acc = 0.0
    ...
    for i in range(n):
        ...
        if td == "long":
            ...
            if buy_signal[i] and position == 0:
                entry = price * (1.0 + slippage_pct)
                shares = cash * 0.95 / entry
                cost = shares * entry * (1.0 + commission)
                if cost <= cash and shares > 0:
                    cash -= cost
                    position = shares
                    total_commission_acc += shares * entry * commission
                    trades.append(
                        {
                ...
                exit_px = price * (1.0 - slippage_pct)
                proceeds = position * exit_px * (1.0 - commission)
                entry_trade = trades[-1]
                entry_cost = float(entry_trade["amount"]) * float(entry_trade["price"])
                pnl = proceeds - (cash_before := cash + entry_cost)  # noqa
                cash += proceeds
                total_commission_acc += position * exit_px * commission
                trades.append(
                    {
                        "time": _ts_to_iso(times[i]),
                        "type": "close_long",
                        "price": round(exit_px, 4),
                        "amount": round(position, 2),
                        "profit": round(pnl, 2),
                        "balance": round(cash, 2),
                    }
                )
        ...
        equity[i] = cash + (position * price if position > 0 else 0)

    # --- Metrics via Polars vectorized operations ---
    eq_series = pl.Series("equity", equity)
    initial_cap = float(initial_capital)
    final_equity = float(equity[-1])

    total_return_pct = (final_equity / initial_cap - 1.0) * 100.0
    total_profit = final_equity - initial_cap

    # Annualized return
    years = max(n / 252.0, 0.02)  # assume ~252 trading days/year
    annual_return = total_return_pct / years if years > 0 else 0.0

    # Sharpe ratio (annualized via Polars)
    returns = eq_series.diff() / eq_series.shift(1).fill_nan(initial_cap)
    returns = returns.fill_nan(0.0)
    rf_daily = 0.02 / 252.0
    excess = returns - rf_daily
    sharpe = float((excess.mean() / excess.std()) * (252.0**0.5)) if excess.std() > 0 else 0.0

    # Max drawdown via Polars cumulative max
    cummax = eq_series.cum_max()
    drawdowns = (eq_series - cummax) / cummax * 100.0
    max_dd = float(drawdowns.min()) if drawdowns.len() > 0 else 0.0

    # Win rate and profit factor from completed round-trips
    completed = []
    round_trips = 0
    i = 0
    while i < len(trades):
        t = trades[i]
        if t["type"] in ("open_long", "close_short"):
            if i + 1 < len(trades):
                next_t = trades[i + 1]
                if next_t["type"] in ("close_long", "open_short"):
                    pnl = float(next_t["profit"])
    ...
    gross_profit = sum(p for p in completed if p > 0)
    gross_loss = abs(sum(p for p in completed if p < 0))
    profit_factor = gross_profit / gross_loss if gross_loss > 0 else (float("inf") if gross_profit > 0 else 1.0)

    # --- Build equity curve in existing format ---
    equity_curve = [{"time": _ts_to_iso(times[i]), "value": round(float(equity[i]), 2)} for i in range(n)]
    ...
    return {
        ...
        "profitFactor": round(profit_factor, 4) if profit_factor != float("inf") else profit_factor,
        "totalTrades": round_trips,
        "totalProfit": round(total_profit, 2),
        "totalCommission": round(total_commission_acc, 2),
        "equityCurve": equity_curve,
        "trades": trades,
    }


def _ts_to_iso(ts_val) -> str:
    """Convert a Unix timestamp (int/float) or datetime to ISO8601 string."""
    if ts_val is None:
        return ""
    try:
        if isinstance(ts_val, (int, float, np.integer, np.floating)):
            return datetime.fromtimestamp(int(ts_val)).strftime("%Y-%m-%d %H:%M")
