...
def _utc_ts_to_user_display(now: int, user_timezone: str) -> tuple[str, str, str]:
    ...
    iso = datetime.fromtimestamp(int(now), tz=UTC).isoformat()
    utz = (user_timezone or "").strip()
    if not utz:
        return iso, iso, "Time (UTC)"
    try:
        dt = datetime.fromtimestamp(int(now), tz=UTC).astimezone(ZoneInfo(utz))
...
class SignalNotifier:
    ...
    def _render_messages(self, payload: dict[str, Any]) -> dict[str, str]:
        ...
        email_html = self._build_email_html(
            title_text="Mipham Quant Signal",
...
"""
    ...
    def _notify_webhook(
        ...
        *,
        ...
        headers: dict[str, str] = {
            ...
            "User-Agent": "MiphamQuant/0.1 (+https://mipham.ai)",
        }
    ...
    def _notify_discord(self, *, url: str, payload: dict[str, Any], fallback_text: str) -> tuple[bool, str]:
        ...
        embed: dict[str, Any] = {
            ...
            "fields": [
                ...
                {
                    "name": "Symbol",
                    "value": str(instrument.get("symbol") or ""),
                    "inline": True,
                },
                {
                    "name": "Signal",
                    "value": str(sig.get("type") or ""),
                    "inline": False,
                },
                {
                    "name": "Price",
                    "value": str(float(order.get("ref_price") or 0.0)),
                    "inline": True,
                },
                {
                    "name": "Stake",
                    "value": str(float(order.get("stake_amount") or 0.0)),
                    "inline": True,
                },
            ],
        }
    ...
    def _notify_telegram(
        ...
        *,
        ...
        if not token:
            return (
                False,
                "missing_telegram_bot_token (请在个人中心配置 Telegram Bot Token)",
            )
        if not chat_id:
    ...
    def send_profile_test_notifications(
        ...
        *,
        ...
        title = "Mipham Quant 通知测试" if zh else "Mipham Quant notification test"
        plain = (
            "这是一条来自 Mipham Quant 个人中心「通知设置」的测试消息。若您收到本条消息，说明该渠道配置正确。"
            if zh
            else "This is a test message from Mipham Quant profile notification settings. "
