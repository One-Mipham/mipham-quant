...
def timeout_context(seconds: int):
    ...
    def _inject_timeout():
        ...
        try:
            ...
            ret = ctypes.pythonapi.PyThreadState_SetAsyncExc(ctypes.c_ulong(target_tid), exc)
            if ret == 0:
...
def safe_exec_with_validation(
    ...
    if not is_safe:
        return {
            "success": False,
            "error": f"Unsafe code rejected: {err}",
            "result": None,
        }
        ...
        if isinstance(node, ast.Import):
            for alias in node.names:
                ...
                if root not in SAFE_IMPORT_MODULES:
                    return (
                        False,
                        f"不允许导入模块 '{alias.name}'，仅允许: {', '.join(sorted(SAFE_IMPORT_MODULES))}",
                    )
            ...
            if node.module:
                ...
                if root not in SAFE_IMPORT_MODULES:
                    return (
                        False,
                        f"不允许导入模块 '{node.module}'，仅允许: {', '.join(sorted(SAFE_IMPORT_MODULES))}",
                    )
            ...
            if isinstance(node.func, ast.Attribute):
                if isinstance(node.func.value, ast.Name) and node.func.value.id in dangerous_modules:
                    return (
                        False,
                        f"检测到危险模块调用: {node.func.value.id}.{node.func.attr}",
                    )
