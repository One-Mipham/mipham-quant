...
from app.data_providers import get_cached, safe_float, set_cached
    ...
    try:
        ...
        for item in symbols[: max(int(limit or 15), 1)]:
            try:
                ...
                logger.debug(
                    "Failed to fetch %s opportunity price %s: %s",
                    m,
                    item.get("symbol"),
                    e,
                )
...
def analyze_opportunities_local_stocks(opportunities: list, market: str):
    ...
    for stock in stock_data:
        ...
        if change > strong_th:
            ...
            reason = f"{market_cn}日涨幅{change:.1f}%，温和上涨"
            impact = "bullish"
        elif change < -strong_th:
            signal, strength = "oversold", "strong"
            reason = f"{market_cn}日跌幅{abs_change:.1f}%，可能超卖反弹"
            impact = "bullish"
        elif change < -medium_th:
            signal, strength = "bearish_momentum", "medium"
            reason = f"{market_cn}日跌幅{abs_change:.1f}%，下跌趋势明显"
            impact = "bearish"
        elif change < -mild_th:
            signal, strength = "bearish_momentum", "weak"
            reason = f"{market_cn}日跌幅{abs_change:.1f}%，温和下跌"
            impact = "bearish"
        elif abs_change <= mild_th:
            signal, strength = "consolidation", "weak"
            reason = f"{market_cn}{name}窄幅震荡({change:+.1f}%)，等待方向选择"
            impact = "neutral"
