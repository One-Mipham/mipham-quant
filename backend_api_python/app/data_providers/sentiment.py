...
def fetch_dollar_index() -> dict[str, Any]:
    ...
    if current > 105:
        level, cn, en = (
            "strong",
            "美元强势 - 利空大宗商品/新兴市场",
            "Strong USD - Bearish commodities/EM",
        )
    elif current > 100:
        level, cn, en = (
            "moderate_strong",
            "美元偏强 - 关注资金流向",
            "Moderately Strong - Watch capital flows",
        )
    elif current > 95:
        ...
        level, cn, en = (
            "moderate_weak",
            "美元偏弱 - 利多风险资产",
            "Moderately Weak - Bullish risk assets",
        )
    else:
        level, cn, en = (
            "weak",
            "美元疲软 - 利多黄金/大宗商品",
            "Weak USD - Bullish gold/commodities",
        )
    ...
    try:
        ...
        if spread < -0.5:
            ...
            level, cn, en, signal = (
                "steep",
                "陡峭曲线 - 经济扩张预期",
                "Steep - Economic expansion expected",
                "bullish",
            )
    ...
    try:
        ...
        if current < 15:
            level, cn, en = (
                "very_low",
                "科技股极低波动 - 市场乐观",
                "Very Low Tech Volatility - Optimistic",
            )
        elif current < 22:
            ...
            level, cn, en = (
                "moderate",
                "科技股中等波动 - 正常",
                "Moderate Tech Volatility - Normal",
            )
        elif current < 35:
            level, cn, en = (
                "high",
                "科技股高波动 - 谨慎",
                "High Tech Volatility - Caution",
            )
        else:
            level, cn, en = (
                "very_high",
                "科技股极高波动 - 恐慌",
                "Very High Tech Volatility - Panic",
            )
    ...
    try:
        ...
        if current < 12:
            level, cn, en = (
                "very_low",
                "黄金低波动 - 避险需求低",
                "Low Gold Vol - Low safe haven demand",
            )
        elif current < 16:
            ...
            level, cn, en = (
                "moderate",
                "黄金中等波动 - 关注避险情绪",
                "Moderate Gold Vol - Watch safe haven",
            )
        elif current < 25:
            level, cn, en = (
                "high",
                "黄金高波动 - 避险需求上升",
                "High Gold Vol - Rising safe haven demand",
            )
        else:
            level, cn, en = (
                "very_high",
                "黄金极高波动 - 市场避险",
                "Very High Gold Vol - Flight to safety",
            )
    ...
    try:
        ...
        if ratio > 1.15:
            level, cn, en, signal = (
                "high_fear",
                "VIX倒挂 - 短期恐慌情绪高涨",
                "VIX Backwardation - High short-term fear",
                "bearish",
            )
        elif ratio > 1.0:
            level, cn, en, signal = (
                "elevated",
                "轻度倒挂 - 市场谨慎",
                "Slight Backwardation - Market cautious",
                "neutral",
            )
        elif ratio > 0.9:
            level, cn, en, signal = (
                "normal",
                "正常结构 - 市场稳定",
                "Normal Structure - Market stable",
                "neutral",
            )
        elif ratio > 0.8:
            level, cn, en, signal = (
                "complacent",
                "深度正价差 - 市场自满",
                "Deep Contango - Market complacent",
                "bullish",
            )
        else:
            level, cn, en, signal = (
