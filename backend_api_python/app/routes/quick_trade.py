...
_FRIENDLY_ERROR_PATTERNS = [
    ...
    (
        _re.compile(
            r"maintenance|unavailable|system.{0,10}(busy|error|upgrade)|suspend|暂停",
            _re.IGNORECASE,
        ),
    ...
    try:
        ...
        if isinstance(raw, dict):
            ...
            if isinstance(data, list) and data and isinstance(data[0], dict):
                ...
                "margin_balance" in data or "margin_available" in data or "withdraw_available" in data
            ):
    ...
    try:
        ...
        if close_scope == "system_tracked" and market_type != "swap":
            return jsonify(
                {
                    "code": 0,
                    "msg": "system_tracked close_scope is only supported for swap/perp",
                }
            ), 400
        ...
        if market_type == "spot":
            ...
            if position_side != "long":
                return jsonify(
                    {
                        "code": 0,
                        "msg": "Spot market only supports closing long positions",
                    }
                ), 400
