...
def list_credentials():
    ...
    try:
        ...
        logger.error(f"list_credentials failed: {e!s}")
...
def create_credential():
    ...
    try:
        ...
        if exchange_id == "ibkr":
            ...
            if not mt5_server or not mt5_login or not mt5_password:
                return jsonify(
                    {
                        "code": 0,
                        "msg": "Missing mt5_server/mt5_login/mt5_password",
                        "data": None,
                    }
                ), 400
            config.update(
        ...
        logger.error(f"create_credential failed: {e!s}")
        logger.error(traceback.format_exc())
...
def delete_credential():
    ...
    try:
        ...
        logger.error(f"delete_credential failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def get_credential():
    ...
    try:
        ...
        logger.error(f"get_credential failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
