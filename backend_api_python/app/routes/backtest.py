...
def run_backtest():
    ...
    try:
        ...
        return jsonify(
            {
                ...
                "data": {"runId": run_id, "result": result},
            }
        )

    except ValueError as e:
        logger.warning(f"Invalid backtest parameters: {e!s}")
        ...
        try:
            ...
            pass
        return jsonify({"code": 0, "msg": f"Backtest failed: {e!s}", "data": None}), 500
...
def _heuristic_ai_advice(runs: list[dict], lang: str) -> str:
    ...
    headings = {
        ...
        "ko-KR": {
            "overall": "요약",
            "params": "파라미터 제안(백테스트 설정 변경)",
            "next": "다음 단계",
        },
        "th-TH": {
            "overall": "สรุป",
            "params": "ข้อเสนอแนะพารามิเตอร์ (ปรับค่าที่ตั้งแบ็กเทสต์)",
            "next": "ขั้นตอนถัดไป",
        },
        "vi-VN": {
            "overall": "Tổng quan",
            ...
            return jsonify(
                {
                    "code": 1,
                    "msg": "OK",
                    "data": {"analysis": analysis, "mode": "heuristic", "lang": lang},
                }
            )
            ...
            return jsonify(
                {
                    "code": 1,
                    "msg": "OK",
                    "data": {"analysis": analysis, "mode": "llm", "lang": lang},
                }
            )
