...
def _strategy_ai_text(key: str, lang: str = "zh-CN") -> str:
    ...
    texts = {
        ...
        if is_zh
        else "Insufficient credits. Please top up and try again.",
...
def _strategy_hint_to_text(hint_code: str, params: dict | None = None, lang: str = "zh-CN") -> str:
    ...
    if hint_code == "NO_CTX_PARAM_DEFAULTS":
        return (
            "没有通过 ctx.param(...) 声明参数默认值。"
            if is_zh
            else "No parameter defaults were declared via ctx.param(...)."
        )
    if hint_code == "NO_ORDER_INTENT":
        return (
            "没有检测到 ctx.buy / ctx.sell / ctx.close_position 等交易动作。"
            if is_zh
            else "No order intent like ctx.buy / ctx.sell / ctx.close_position was detected."
        )
...
def _strategy_human_summary(
    ...
    if auto_fix_applied and auto_fix_succeeded:
        title = (
            "AI 已自动修复并返回更稳定的策略代码"
            if is_zh
            else "AI auto-fixed the strategy code and returned a more stable version"
        )
    elif auto_fix_applied:
        title = (
            "AI 尝试自动修复策略代码，但仍保留部分问题"
            if is_zh
            else "AI attempted to auto-fix the strategy code, but some issues still remain"
        )
    else:
        title = (
            "AI 已生成策略代码，并通过当前质检流程"
            if is_zh
            else "AI generated strategy code and it passed the current QA flow"
        )

    returned_text = (
        "当前返回的是自动修复后的代码。"
        if returned_candidate == "repaired" and is_zh
        else "The returned code is the auto-fixed version."
...
def list_strategies():
    ...
    try:
        ...
        logger.error(f"list_strategies failed: {e!s}")
...
def get_strategy_detail():
    try:
        ...
        logger.error(f"get_strategy_detail failed: {e!s}")
        logger.error(traceback.format_exc())
...
def run_strategy_backtest():
    try:
        ...
        logger.error(f"run_strategy_backtest failed: {e!s}")
        logger.error(traceback.format_exc())
        try:
...
def get_strategy_backtest_history():
    try:
        ...
        logger.error(f"get_strategy_backtest_history failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def get_strategy_backtest_run():
    try:
        ...
        logger.error(f"get_strategy_backtest_run failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def create_strategy():
    try:
        ...
        logger.error(f"create_strategy failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def batch_create_strategies():
    ...
    try:
        ...
        logger.error(f"batch_create_strategies failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def batch_start_strategies():
    ...
    try:
        ...
        logger.error(f"batch_start_strategies failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def batch_stop_strategies():
    ...
    try:
        ...
        logger.error(f"batch_stop_strategies failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def batch_delete_strategies():
    ...
    try:
        ...
        logger.error(f"batch_delete_strategies failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def update_strategy():
    try:
        ...
        logger.error(f"update_strategy failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def delete_strategy():
    try:
        ...
        logger.error(f"delete_strategy failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
        ...
        return jsonify(
            {
                "code": 1,
                "msg": "success",
                "data": {"trades": processed_rows, "items": processed_rows},
            }
        )
    except Exception as e:
        logger.error(f"get_trades failed: {e!s}")
        logger.error(traceback.format_exc())
...
def get_positions():
    ...
    try:
        ...
        logger.error(f"get_positions failed: {e!s}")
        logger.error(traceback.format_exc())
...
def get_equity_curve():
    ...
    try:
        ...
        logger.error(f"get_equity_curve failed: {e!s}")
        logger.error(traceback.format_exc())
...
def stop_strategy():
    ...
    try:
        ...
        logger.error(f"Failed to stop strategy: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": f"Failed to stop strategy: {e!s}", "data": None}), 500
...
def start_strategy():
    ...
    try:
        ...
        logger.error(f"Failed to start strategy: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": f"Failed to start strategy: {e!s}", "data": None}), 500
    ...
    try:
        ...
        if not isinstance(exchange_config, dict):
            ...
            return jsonify(
                {
                    "code": 0,
                    "msg": "Invalid exchange config format; please check your payload",
                    "data": None,
                }
            )
        ...
        if ex_id in ("ibkr", "mt5"):
            if not local_desktop_brokers_allowed():
                return jsonify(
                    {
                        "code": 0,
                        "msg": desktop_broker_cloud_reject_message(),
                        "data": None,
                    }
                )
            logger.info(
                "Testing connection: exchange_id=%s (local desktop broker, skipping API key check)",
                ex_id,
            )
        else:
            ...
            if not api_key or not secret_key:
                return jsonify(
                    {
                        "code": 0,
                        "msg": "Please provide API key and secret key",
                        "data": None,
                    }
                )
        ...
        if result["success"]:
            return jsonify(
                {
                    "code": 1,
                    "msg": result.get("message") or "Connection successful",
        ...
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": f"Connection test failed: {e!s}", "data": None}), 500
...
def get_symbols():
    ...
    try:
        ...
        return jsonify(
            {
                "code": 0,
                "msg": f"Failed to fetch symbols: {e!s}",
                "data": {"symbols": []},
            }
        ), 500
...
def preview_compile():
    ...
    try:
        ...
        try:
            ...
            return jsonify({"code": 0, "msg": f"Compilation failed: {e!s}"}), 400
...
def get_strategy_notifications():
    ...
    try:
        ...
        logger.error(f"get_strategy_notifications failed: {e!s}")
        logger.error(traceback.format_exc())
...
def get_unread_notification_count():
    ...
    try:
        ...
        logger.error(f"get_unread_notification_count failed: {e!s}")
        logger.error(traceback.format_exc())
...
def mark_notification_read():
    ...
    try:
        ...
        logger.error(f"mark_notification_read failed: {e!s}")
        return jsonify({"code": 0, "msg": str(e)}), 500
...
def mark_all_notifications_read():
    ...
    try:
        ...
        logger.error(f"mark_all_notifications_read failed: {e!s}")
        return jsonify({"code": 0, "msg": str(e)}), 500
...
def clear_notifications():
    ...
    try:
        ...
        logger.error(f"clear_notifications failed: {e!s}")
        return jsonify({"code": 0, "msg": str(e)}), 500
...
def verify_strategy_code():
    ...
    try:
        ...
        logger.error(f"verify_strategy_code failed: {e!s}")
    ...
    try:
        ...
        if not prompt.strip():
            return jsonify(
                {
                    "code": "",
                    "msg": _strategy_ai_text("prompt_empty", lang),
                    "params": None,
                }
            )
        ...
        if not api_key:
            return jsonify(
                {
                    "code": "",
                    "msg": _strategy_ai_text("no_llm_key", lang),
                    "params": None,
                }
            )
        ...
        if not ok:
            msg = (
                f"积分不足: {billing_msg}"
                if _is_zh_lang(lang) and billing_msg
                else _strategy_ai_text("insufficient_credits", lang)
            )
        ...
        if intent == "bot_recommend":
            ...
            try:
                ...
                if detected_symbol:
                    ...
                    ks = KlineService()
                    klines_4h = ks.get_kline(
                        market="Crypto",
                        symbol=detected_symbol,
                        timeframe="4h",
                        limit=50,
                    )
                    klines_1d = ks.get_kline(
                        market="Crypto",
                        symbol=detected_symbol,
                        timeframe="1d",
                        limit=30,
                    ...
                    if klines and len(klines) >= 5:
                        ...
                        logger.info(
                            f"[AI Bot] Fetched market data for {detected_symbol}: price={current_price}, "
                ...
                return jsonify(
                    {
                        "code": "",
                        "params": None,
                        "msg": _strategy_ai_text("invalid_json_params", lang),
                    }
                )
            return jsonify(
                {
                    "code": "",
                    "params": updates,
                    "msg": _strategy_ai_text("success", lang),
                }
            )
...
"""
        ...
        def _repair_strategy_code_via_llm(bad_code: str, validation: dict) -> str:
            repair_prompt = (
                "You produced Mipham Quant strategy script code that failed automatic validation. "
                ...
                "- Must compile and run in Mipham Quant strategy runtime.\n"
        ...
        if _needs_auto_fix_strategy(validation):
            logger.warning(
                "ai_generate_strategy produced code needing auto-fix: %s",
                _format_strategy_validation_issues(validation),
            )
            try:
        ...
        if content:
            ...
            return jsonify(
                {
                    "code": "",
                    "msg": _strategy_ai_text("ai_empty_result", lang),
                    "params": None,
                    "debug": debug,
                }
            )
    except Exception as e:
        logger.error(f"ai_generate_strategy failed: {e!s}")
...
def get_strategy_performance():
    ...
    try:
        ...
        logger.error(f"get_strategy_performance failed: {e!s}")
        return jsonify({"code": 0, "msg": str(e)}), 500
...
def get_strategy_logs():
    ...
    try:
        ...
        for r in rows or []:
            ...
            if ts is not None and hasattr(ts, "isoformat"):
                if getattr(ts, "tzinfo", None) is not None:
                    rr["timestamp"] = ts.astimezone(UTC).strftime("%Y-%m-%dT%H:%M:%S") + "Z"
                else:
        ...
        if "qd_strategy_logs" in el and ("does not exist" in el or "no such table" in el):
            return jsonify({"code": 1, "msg": "success", "data": []})
        logger.error(f"get_strategy_logs failed: {e!s}")
        return jsonify({"code": 0, "msg": str(e)}), 500
