...
def get_public_config():
    ...
    Mirrors the old PHP `/addons/mipham-quant/index/getConfig` shape.
    """
    try:
        ...
        logger.error(f"get_public_config failed: {e!s}")
...
def search_symbols():
    ...
    try:
        ...
        logger.error(f"search_symbols failed: {e!s}")
...
def _search_crypto_exchange(keyword: str, limit: int, existing: set) -> list:
    ...
    try:
        ...
        if _crypto_markets_cache["data"] and now - _crypto_markets_cache["ts"] < 14400:
            ...
            logger.info(
                "Cached %d USDT crypto pairs from %s",
                len(markets),
                CCXTConfig.DEFAULT_EXCHANGE,
            )
...
def get_hot_symbols():
    ...
    try:
        ...
        logger.error(f"get_hot_symbols failed: {e!s}")
        return jsonify({"code": 0, "msg": str(e), "data": []}), 500
...
def get_watchlist():
    ...
    try:
        ...
        logger.error(f"get_watchlist failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": []}), 500
...
def add_watchlist():
    ...
    try:
        ...
        logger.error(f"add_watchlist failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def remove_watchlist():
    ...
    try:
        ...
        logger.error(f"remove_watchlist failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def get_watchlist_prices():
    ...
    try:
        ...
        try:
            for future in as_completed(futures, timeout=30):
                ...
                try:
                    ...
                    logger.warning(f"Price fetch failed: {market}:{symbol} - {e!s}")
                    results.append(
                        {
                            "market": market,
                            "symbol": symbol,
        ...
        logger.error(f"Batch watchlist price fetch failed: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": f"Failed: {e!s}", "data": []}), 500
...
def get_price():
    ...
    try:
        ...
        logger.error(f"Failed to fetch price: {e!s}")
        return jsonify({"code": 0, "msg": f"Failed: {e!s}", "data": None}), 500
...
def get_stock_name():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Failed to fetch stock name; falling back to symbol: {market}:{symbol} - {e!s}")
            stock_name = symbol
        ...
        logger.error(f"Failed to fetch stock name: {e!s}")
        logger.error(traceback.format_exc())
        return jsonify({"code": 0, "msg": f"Failed: {e!s}", "data": None}), 500
