...
`/addons/mipham-quant/indicator/getIndicators`.
...
def _indicator_hint_to_text(hint_code: str, params: dict[str, Any] | None = None, lang: str = "zh-CN") -> str:
    ...
    if hint_code == "DECLARED_PARAMS_NOT_READ_VIA_PARAMS_GET":
        ...
        return (
            ...
            if is_zh
            else f"Declared parameters are not being read via params.get(...): {joined}."
        )
    if hint_code == "SIGNAL_MARKERS_USE_WHERE_NONE":
        return (
            "已检测到信号标记使用 where(..., None).tolist()，建议改为显式 None 列表以避免 NaN 渲染问题。"
            if is_zh
            else "Signal markers use where(..., None).tolist(); prefer an explicit None list to avoid NaN rendering issues."
        )
    ...
    if hint_code == "UNKNOWN_STRATEGY_KEY":
        ...
        return f"存在未知的 @strategy 键：{key}。" if is_zh else f"Unknown @strategy key detected: {key}."
...
def _indicator_human_summary(
    ...
    if returned_candidate == "repaired":
        ...
        returned_text = (
            "当前返回的是首次生成的代码。" if is_zh else "The returned code is the initially generated version."
        )
...
def get_indicators():
    ...
    try:
        ...
        logger.error(f"get_indicators failed: {e!s}", exc_info=True)
        ...
        return jsonify(
            {
                "code": 1,
                "msg": "success",
                "data": {"id": indicator_id, "userid": user_id},
            }
        )
    except Exception as e:
        logger.error(f"save_indicator failed: {e!s}", exc_info=True)
...
def delete_indicator():
    ...
    try:
        ...
        logger.error(f"delete_indicator failed: {e!s}", exc_info=True)
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def get_indicator_params():
    ...
    try:
        ...
        logger.error(f"get_indicator_params failed: {e!s}", exc_info=True)
        return jsonify({"code": 0, "msg": str(e), "data": None}), 500
...
def verify_code():
    ...
    try:
        ...
        logger.error(f"verify_code failed: {e!s}", exc_info=True)
        return jsonify({"code": 0, "msg": f"System Error: {e!s}", "data": None}), 500
...
def ai_generate():
    ...
    if not prompt:
        ...
        def _err_stream():
            yield (
                "data: "
                + json.dumps(
                    {"error": _indicator_ai_text("prompt_required", lang)},
                    ensure_ascii=False,
                )
                + "\n\n"
            )
    ...
    # Mipham Quant indicator IDE: chart render + backtest; must pass server verifyCode + safe_exec rules.
    SYSTEM_PROMPT = """# Role

You write production-ready **Mipham Quant** indicator scripts: Python that runs in the Indicator IDE, renders on the K-line chart, and drives **backtest entries/exits** via boolean signals. Code must be syntactically valid, safe for the host sandbox, and match the exact I/O contract below.
...
"""
    ...
    def _generate_code_via_llm() -> str:
        ...
        if existing:
            user_prompt = (
                "# Existing Mipham Quant indicator code (keep working output/buy/sell contract):\n\n```python\n"
                + existing.strip()
                + "\n```\n\n# Change request:\n\n"
                + prompt
                + "\n\nReturn one full replacement script: same Mipham Quant rules (my_indicator_name/description, df = df.copy(), declared @param values must be read via params.get(...), df['buy']/df['sell'], output dict, list lengths == len(df)). "
    ...
    def _repair_code_via_llm(bad_code: str, validation: dict[str, Any]) -> str:
        ...
        repair_prompt = (
            "You produced Mipham Quant indicator code that failed automatic validation. "
            ...
            "# Current code\n```python\n" + bad_code.strip() + "\n```\n\n"
            "# Repair requirements\n"
            "- Keep Mipham Quant indicator contract intact.\n"
    ...
    def _generate_final_code() -> tuple[str, dict[str, Any]]:
        ...
        logger.warning(
            "ai_generate produced code needing auto-fix: %s",
            _format_validation_issues(validation),
        )
        try:
            ...
            debug["human_summary"] = _indicator_human_summary(validation, validation, True, False, "initial", lang=lang)
            logger.info("ai_generate debug=%s", json.dumps(debug, ensure_ascii=False))
            return code_text, debug
    ...
    def stream():
        ...
        if not ok:
            error_msg = (
                f"积分不足: {msg}" if _is_zh_lang(lang) and msg else _indicator_ai_text("insufficient_credits", lang)
            )
        ...
        return jsonify(
            {
                "code": 1,
                "msg": "success",
                "data": {"df": result_dict, "columns": list(result_df.columns)},
            }
        )
