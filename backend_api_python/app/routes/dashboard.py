...
def summary():
    ...
    try:
        ...
        hourly_distribution = [
            {
                "hour": h,
                "count": hour_to_count.get(h, 0),
                "profit": round(hour_to_profit.get(h, 0.0), 2),
            }
            for h in range(24)
        ]
...
def pending_orders():
    ...
    try:
        ...
        with get_db_connection() as db:
            cur = db.cursor()
            cur.execute(
                "SELECT COUNT(1) AS cnt FROM pending_orders WHERE user_id = ?",
                (user_id,),
            )
...
def delete_pending_order(order_id: int):
    ...
    try:
        ...
        with get_db_connection() as db:
            ...
            cur.execute(
                "SELECT id, status FROM pending_orders WHERE id = ? AND user_id = ?",
                (oid, user_id),
            )
            row = cur.fetchone() or {}
            if not row:
