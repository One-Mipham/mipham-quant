...
def export_users():
    ...
    try:
        ...
        filename = "mipham_quant_users_export.csv"
        return Response(
            output.getvalue(),
...
def get_roles():
    ...
    return jsonify({"code": 1, "msg": "success", "data": {"roles": roles}})
    ...
    try:
        ...
        if credits is None or credits < 0:
            return jsonify(
                {
                    "code": 0,
                    "msg": "Credits must be a non-negative number",
                    "data": None,
                }
            ), 400
...
def set_user_vip():
    ...
    try:
        ...
        if vip_expires_at_str:
            ...
            if vip_days > 0:
                expires_at = datetime.now(UTC) + timedelta(days=vip_days)
            else:
    ...
    try:
        ...
        if "timezone" in data:
            ...
            if tz and (len(tz) > 64 or not _PROFILE_TIMEZONE_RE.match(tz)):
                return jsonify({"code": 0, "msg": "Invalid timezone identifier", "data": None}), 400
            allowed["timezone"] = tz
...
def get_my_referrals():
    ...
    try:
        ...
        with get_db_connection() as db:
            ...
            # Get total count
            cur.execute("SELECT COUNT(*) as cnt FROM qd_users WHERE referred_by = ?", (user_id,))
...
def get_notification_settings():
    ...
    try:
        ...
        with get_db_connection() as db:
            cur = db.cursor()
            cur.execute(
                "SELECT notification_settings, email FROM qd_users WHERE id = ?",
                (user_id,),
            )
            row = cur.fetchone()
            cur.close()
...
def delete_chart_template():
    ...
    try:
        ...
        with get_db_connection() as db:
            ...
            updated_templates = [
                tpl for tpl in templates if not (isinstance(tpl, dict) and str(tpl.get("id") or "") == template_id)
            ]
            cur.execute(
                "UPDATE qd_users SET chart_templates = ?, updated_at = NOW() WHERE id = ?",
                (json.dumps(updated_templates, ensure_ascii=False), user_id),
...
def get_system_strategies():
    ...
    try:
        ...
        for s in strategies:
            ...
            if not indicator_name and str(s.get("strategy_mode") or "").strip().lower() == "bot":
                if isinstance(trading_config, dict):
                    indicator_name = (
                        trading_config.get("bot_name") or s.get("strategy_name") or trading_config.get("bot_type") or ""
                    )
                else:
        ...
        total_system_pnl = float(unreal_row.get("total_unrealized") or 0) + float(
            realized_row.get("total_realized") or 0
        )
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "summary": {
                        ...
                        "total_roi": round(
                            (total_system_pnl / total_capital * 100) if total_capital > 0 else 0,
                            2,
                        ),
