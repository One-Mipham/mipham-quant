        ...
        "items": [
            {
                ...
                "label": "Secret Key",
                "type": "password",
                "default": "mipham-quant-secret-key-change-me",
...
def save_settings():
    ...
    try:
        ...
        return jsonify({"code": 0, "msg": f"Save failed: {e!s}"})
...
def get_openrouter_balance():
    ...
    try:
        ...
        if not api_key:
            return jsonify({"code": 0, "msg": "OpenRouter API Key 未配置", "data": None})
        ...
        if resp.status_code == 200:
            ...
            return jsonify({"code": 0, "msg": "API Key 无效或已过期", "data": None})
        else:
            return jsonify({"code": 0, "msg": f"查询失败: HTTP {resp.status_code}", "data": None})

    except requests.exceptions.Timeout:
        return jsonify({"code": 0, "msg": "请求超时，请检查网络连接", "data": None})
    except Exception as e:
        logger.error(f"Get OpenRouter balance failed: {e}")
        return jsonify({"code": 0, "msg": f"查询失败: {e!s}", "data": None})
...
def test_connection():
    ...
    try:
        ...
        return jsonify({"code": 0, "msg": f"Test failed: {e!s}"})
