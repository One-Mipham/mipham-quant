...
from app.data_providers import clear_cache, get_cached, set_cached
    ...
    try:
        ...
        if cached:
            logger.debug(
                ...
                len(cached.get("indices", [])),
                len(cached.get("forex", [])),
                len(cached.get("crypto", [])),
                len(cached.get("commodities", [])),
            )
...
def trading_opportunities():
    ...
    try:
        ...
        scanners = [
            ...
            (
                "CNStock",
                lambda: analyze_opportunities_local_stocks(opportunities, "CNStock"),
            ),
            (
                "HKStock",
                lambda: analyze_opportunities_local_stocks(opportunities, "HKStock"),
            ),
