...
class PolymarketDataSource:
    ...
    def _get_cached_markets(self, category: str = None, limit: int = 50) -> list[dict] | None:
        """从数据库缓存读取市场数据"""
        try:
            with get_db_connection() as db:
                cur = db.cursor()
    ...
    def _fetch_markets_from_api(self, category: str = None, limit: int = 50) -> list[dict]:
        ...
        try:
            ...
            logger.warning(
                f"Gamma API failed to fetch markets for category '{category}' (可能原因: API暂时不可用、网络问题、限流或返回空数据)"
            )
            return []
    ...
    def _fetch_from_gamma_api(self, category: str = None, limit: int = 50) -> list[dict]:
        ...
        try:
            ...
            if status_code == 429:
                logger.warning("Gamma API rate limited (429). 建议: 稍后重试或减少请求频率")
            elif status_code == 503:
                logger.warning("Gamma API service unavailable (503). Polymarket API可能正在维护")
    ...
    def _build_polymarket_url(self, slug: str | None, market_id: str) -> str:
        """
    ...
    def _fetch_market_detail_by_id(self, market_id: str) -> dict | None:
        """
    ...
    def _fetch_market_from_api(self, market_id: str) -> dict | None:
        """
        从Gamma API获取单个市场数据
        支持通过slug或id查询
        """
