...
def _float_clean(x: Any) -> float | None:
    if x is None or x == "":
        return None
    # AKShare THS returns Python False for N/A values — treat as missing
...
def _td_request(endpoint: str, symbol: str, exchange: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if data.get("status") == "error":
                ...
                if code == 429 or "API credits" in msg or "minute limit" in msg:
                    logger.warning(
                        "TwelveData rate limit on %s %s/%s: %s",
                        endpoint,
                        symbol,
                        exchange,
                        msg,
...
def fetch_twelvedata_statements(tencent_code: str, is_hk: bool) -> dict[str, Any]:
    ...
    logger.debug(
        "TwelveData statements %s/%s: %d statement sections, %d indicators",
        symbol,
        exchange,
        len(statements),
        len(result),
...
def fetch_twelvedata_profile(tencent_code: str, is_hk: bool) -> dict[str, Any]:
    ...
    logger.debug(
        "TwelveData /profile %s/%s: name=%s industry=%s",
        symbol,
        exchange,
        out.get("name"),
        out.get("industry"),
...
def _safe_iloc(df, row: int, col: str) -> float | None:
...
def fetch_cn_financial_indicators(tencent_code: str) -> dict[str, Any]:
    """
    Fetch revenue growth, debt/equity, current ratio, ROE, margins from
    stock_financial_abstract_ths (同花顺财务摘要).

    This is the most reliable free A-share fundamental endpoint — works
    globally and provides 25 indicators going back decades.

    Fields extracted:
      - revenue_growth, earnings_growth, profit_margin, gross_margin
      - roe, roe_diluted, eps, book_value_per_share
      - current_ratio, quick_ratio, debt_to_equity (derived from 资产负债率)
      - operating_cash_flow (per share)
    """
    sym6 = ak_a_code_from_tencent(tencent_code)
    if not sym6:
        return {}
    ...
    try:
        ...
        with _bypass_proxy():
            df = ak.stock_financial_abstract_ths(symbol=sym6, indicator="按报告期")
    except ImportError:
        logger.warning("akshare not installed, A-share financial data unavailable")
        return {}
    except Exception as e:
        logger.debug("stock_financial_abstract_ths failed %s: %s", sym6, e)
        return {}

    if df is None or df.empty:
        return result

    # Use the latest row (row 0) — THS puts most recent at position 0
    curr = df.iloc[0]

    # --- Growth & returns: percentages, use _parse_pct directly ---
    result["revenue_growth"] = _parse_pct(curr.get("营业总收入同比增长率"))
    result["earnings_growth"] = _parse_pct(curr.get("净利润同比增长率"))
    result["profit_margin"] = _parse_pct(curr.get("销售净利率"))
    result["gross_margin"] = _parse_pct(curr.get("销售毛利率"))
    result["roe"] = _parse_pct(curr.get("净资产收益率"))
    result["roe_diluted"] = _parse_pct(curr.get("净资产收益率-摊薄"))

    # --- Per-share: plain numbers (or False for N/A) ---
    result["eps"] = _float_clean(curr.get("基本每股收益"))
    result["book_value_per_share"] = _float_clean(curr.get("每股净资产"))
    result["operating_cash_flow_per_share"] = _float_clean(curr.get("每股经营现金流"))

    # --- Liquidity & efficiency: plain numbers ---
    result["current_ratio"] = _float_clean(curr.get("流动比率"))
    result["quick_ratio"] = _float_clean(curr.get("速动比率"))
    result["inventory_turnover"] = _float_clean(curr.get("存货周转率"))
    result["inventory_turnover_days"] = _float_clean(curr.get("存货周转天数"))
    result["receivables_turnover_days"] = _float_clean(curr.get("应收账款周转天数"))

    # --- Leverage: debt_to_equity derived from 资产负债率 ---
    dr = _parse_pct(curr.get("资产负债率"))
    if dr is not None and 0 < dr < 100:
        result["debt_to_equity"] = round(dr / (100 - dr), 4)

    if result:
        logger.debug("A-share financial indicators (THS) for %s: %d fields", sym6, len(result))
    return result


def _parse_pct(value) -> float | None:
    """Parse a percentage value from AKShare THS — handles '52.49%', False, and float.

    Returns the percentage as a number (e.g. '52.49%' → 52.49).
    Returns None if the value is N/A (False, None, empty, etc.).
    """
    if value is None:
        return None
    if isinstance(value, bool):
        # AKShare returns False for N/A
        return None
        ...
        with _bypass_proxy():
            try:
                ...
                if profit_df is not None and not profit_df.empty:
                    r = profit_df.iloc[0]
                    date_col = next(
                        (c for c in ("报告日期", "REPORT_DATE_NAME", "REPORT_DATE") if c in profit_df.columns),
                        None,
                    )
                    statements["income_statement"] = {
            ...
            try:
                ...
                if balance_df is not None and not balance_df.empty:
                    r = balance_df.iloc[0]
                    date_col = next(
                        (c for c in ("报告日期", "REPORT_DATE_NAME", "REPORT_DATE") if c in balance_df.columns),
                        None,
                    )
                    statements["balance_sheet"] = {
                        "latest_date": str(r[date_col])[:10] if date_col and r.get(date_col) is not None else None,
                        ...
                        "stockholders_equity": _float_clean(r.get("股东权益合计"))
                        or _float_clean(r.get("所有者权益合计")),
            ...
            try:
                ...
                if cashflow_df is not None and not cashflow_df.empty:
                    ...
                    date_col = next(
                        (c for c in ("报告日期", "REPORT_DATE_NAME", "REPORT_DATE") if c in cashflow_df.columns),
                        None,
                    )
                    ...
                    statements["cash_flow"] = {
                        ...
                        "free_cash_flow": round(op_cf - abs(capex), 2)
...
# ---------------------------------------------------------------------------
# Comprehensive A-share fundamentals — AKShare direct (free, Eastmoney)
# ---------------------------------------------------------------------------


def fetch_akshare_cn_fundamentals(tencent_code: str) -> dict[str, Any]:
    """
    Fetch comprehensive A-share fundamental data via AKShare (Eastmoney).

    Uses targeted AKShare APIs (NOT the heavy stock_zh_a_spot_em)
    to retrieve: pe_ratio, pb_ratio, roe, roa, revenue_growth,
    profit_growth, market_cap, dividend_yield, total_shares.

    Args:
        tencent_code: Tencent-style code like 'SH600519' or 'SZ000001'

    Returns:
        Dict with fundamental fields. Empty dict on failure.
    """
    sym6 = ak_a_code_from_tencent(tencent_code)
    if not sym6:
        return {}

    result: dict[str, Any] = {"source": "akshare_cn_fundamentals"}
    em_sym = _eastmoney_a_em_symbol(tencent_code)

    try:
        import akshare as ak  # type: ignore
    except ImportError:
        logger.debug("akshare not installed; skipping CN fundamentals")
        return {}

    # --- 1. Market cap, total shares, industry from stock_individual_info_em ---
    info = _individual_info_map(sym6)
    if info:
        result["market_cap"] = _float_clean(info.get("总市值"))
        result["float_market_cap"] = _float_clean(info.get("流通市值"))
    ...
    # --- 2. PE / PB / PS / PEG from stock_zh_valuation_comparison_em ---
    try:
        with _bypass_proxy():
            vdf = ak.stock_zh_valuation_comparison_em(symbol=em_sym)
        if vdf is not None and not vdf.empty and "代码" in vdf.columns:
            ...
            if not hit.empty:
                ...
                if peg is not None:
                    result["peg"] = peg
                # 股息率 (if available in this endpoint)
                div_yield = _float_clean(r.get("股息率"))
                if div_yield is not None:
                    result["dividend_yield"] = div_yield
    except Exception as e:
        logger.debug("stock_zh_valuation_comparison_em failed %s: %s", em_sym, e)

    # --- 3. Rich financial indicators from stock_financial_abstract_ths ---
    try:
        with _bypass_proxy():
            fin_abs = ak.stock_financial_abstract_ths(symbol=sym6, indicator="按报告期")
        if fin_abs is not None and not fin_abs.empty:
            latest = fin_abs.iloc[0]

            # Percentage fields — use _parse_pct directly (handles '54.27%', False, etc.)
            result["roe"] = _parse_pct(latest.get("净资产收益率"))
            result["roa"] = _parse_pct(latest.get("总资产收益率") or latest.get("总资产报酬率"))
            result["revenue_growth"] = _parse_pct(latest.get("营业总收入同比增长率"))
            result["profit_growth"] = _parse_pct(latest.get("净利润同比增长率"))
            result["profit_margin"] = _parse_pct(latest.get("销售净利率"))
            result["gross_margin"] = _parse_pct(latest.get("销售毛利率"))

            # Ratio fields — _float_clean is safe (they are plain numbers like '1.16')
            result["eps"] = _float_clean(latest.get("基本每股收益"))
            result["book_value_per_share"] = _float_clean(latest.get("每股净资产"))
            result["current_ratio"] = _float_clean(latest.get("流动比率"))
            result["quick_ratio"] = _float_clean(latest.get("速动比率"))
            result["operating_cash_flow_per_share"] = _float_clean(latest.get("每股经营现金流"))
            result["inventory_turnover"] = _float_clean(latest.get("存货周转率"))
            result["inventory_turnover_days"] = _float_clean(latest.get("存货周转天数"))

            # Debt-to-equity: derive from 资产负债率 (debt ratio %)
            dr = _parse_pct(latest.get("资产负债率"))
            if dr is not None and 0 < dr < 100:
                result["debt_to_equity"] = round(dr / (100 - dr), 4)
    except Exception as e:
        logger.debug("stock_financial_abstract_ths failed %s: %s", sym6, e)

    non_null = sum(1 for v in result.values() if v is not None and v != "akshare_cn_fundamentals")
    logger.debug("AKShare CN fundamentals for %s: %d non-null fields", sym6, non_null)
    return result


# ---------------------------------------------------------------------------
# Top-level aggregators — multi-tier fundamentals
# ---------------------------------------------------------------------------


def get_cn_fundamentals(tencent_code: str) -> dict[str, Any]:
    """
    Get A-share fundamentals with multi-tier fallback.

    Priority:
      1. AKShare (free, direct Eastmoney, most comprehensive for A-shares)
      2. Twelve Data (paid, globally stable, works from overseas)

    Returns merged result from all available sources.
    """
    result: dict[str, Any] = {}

    # Try 1: AKShare — free, most comprehensive for A-shares
    ak_result = fetch_akshare_cn_fundamentals(tencent_code)
    if ak_result:
        result.update(ak_result)

    # Try 2: Twelve Data — paid, works globally, fills in gaps
    td_result = fetch_twelvedata_fundamental(tencent_code, is_hk=False)
    if td_result:
        # Merge — don't overwrite existing non-null AKShare values
        for key, val in td_result.items():
            if key in ("source",):
                continue
            if val is not None and (key not in result or result[key] is None):
                result[key] = val
        if not result.get("source"):
            result["source"] = td_result.get("source")

    return result


def get_hk_fundamentals(tencent_code: str) -> dict[str, Any]:
    """
    Get HK stock fundamentals with multi-tier fallback.

    Priority:
      1. Twelve Data (paid, globally stable)
      2. AKShare HK (free, Eastmoney, Chinese domestic)

    Returns merged result from all available sources.
    """
    result: dict[str, Any] = {}

    # Try 1: Twelve Data — most reliable for HK
    td_result = fetch_twelvedata_fundamental(tencent_code, is_hk=True)
    if td_result:
        result.update(td_result)

    # Try 2: AKShare HK — supplements PE/PB/ROE/div
    ak_result = fetch_hk_fundamental_akshare(tencent_code)
    if ak_result:
        for key, val in ak_result.items():
            if key in ("source",):
                continue
            if val is not None and (key not in result or result[key] is None):
                result[key] = val
        if not result.get("source"):
            result["source"] = ak_result.get("source")
