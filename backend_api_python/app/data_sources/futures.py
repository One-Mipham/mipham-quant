...
from app.config import APIKeys, CCXTConfig, TiingoConfig
...
class FuturesDataSource(BaseDataSource):
    ...
    def _get_ticker_twelvedata(self, symbol: str) -> dict[str, Any] | None:
    ...
    def _get_ticker_tiingo(self, symbol: str) -> dict[str, Any] | None:
        ...
        try:
            resp = requests.get(
                f"{TiingoConfig.BASE_URL}/fx/top",
                params={
                    "tickers": tiingo_sym,
                    "token": api_key,
                },
    ...
    def _get_traditional_futures(
        self, symbol: str, timeframe: str, limit: int, before_time: int | None = None
    ) -> list[dict[str, Any]]:
    ...
    def _get_traditional_futures_td(
        ...
        try:
            ...
            if data.get("status") == "error":
                logger.debug(
                    "TwelveData futures kline error %s: %s",
                    td_sym,
                    data.get("message", ""),
                )
                return []
            ...
            if len(klines) > limit:
                klines = klines[-limit:]
            logger.debug(
                "TwelveData futures kline %s %s: %d bars",
                td_sym,
                timeframe,
                len(klines),
            )
    ...
    def _get_traditional_futures_tiingo(
        ...
        try:
            ...
            resp = requests.get(
                f"{TiingoConfig.BASE_URL}/fx/{tiingo_sym}/prices",
                params={
            ...
            if len(klines) > limit:
                klines = klines[-limit:]
            logger.debug(
                "Tiingo futures kline %s %s: %d bars",
                tiingo_sym,
                timeframe,
                len(klines),
            )
