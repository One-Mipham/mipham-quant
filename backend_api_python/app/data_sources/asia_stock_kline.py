    ...
    *,
    ...
    for attempt in range(_MAX_ATTEMPTS):
        try:
            ...
            if attempt + 1 < _MAX_ATTEMPTS and _is_transient(e):
                ...
                continue
            logger.warning(
                "TwelveData request failed %s/%s tf=%s: %s",
                symbol,
                exchange,
                timeframe,
                e,
    ...
    if data.get("status") != "ok" or "values" not in data:
        ...
        if code == 429 or "API credits" in msg or "minute limit" in msg:
            ...
            logger.debug(
                "TwelveData plan limit %s/%s tf=%s: %s",
                symbol,
                exchange,
                timeframe,
                msg,
    ...
    logger.debug(
        "TwelveData returned %d bars for %s/%s tf=%s",
        len(out),
        symbol,
        exchange,
        timeframe,
...
def _minute_period_str(timeframe: str) -> str | None:
    return {"1m": "1", "5m": "5", "15m": "15", "30m": "30", "1H": "60", "4H": "60"}.get(timeframe)


def _min_bar_window(timeframe: str, limit: int, before_time: int | None) -> tuple[str, str]:
    _ = (timeframe, limit)
    end = datetime.fromtimestamp(int(before_time)) if before_time else datetime.now()
    ...
    for attempt in range(_MAX_ATTEMPTS):
        try:
            ...
            if attempt + 1 < _MAX_ATTEMPTS and _is_transient(e):
                ...
                continue
            logger.warning(
                "AkShare minute K-line failed %s tf=%s sym=%s: %s",
                tencent_code,
                timeframe,
                sym,
                e,
...
# ---------------------------------------------------------------------------
# Futu OpenAPI — HK stock klines (best quality, real-time capable)
# Requires FutuOpenD running locally on port 11111
# ---------------------------------------------------------------------------


def _is_port_open(host: str, port: int, timeout: float = 2.0) -> bool:
    """Quick TCP connectivity check — returns True if port is reachable."""
    try:
...
def fetch_futu_hk_klines(
    ...
    Requires FutuOpenD running locally (default host=127.0.0.1, port=11111).
    Returns empty list if futu-api not installed or FutuOpenD not running.

    Args:
        symbol: HK stock code like 'HK.00700' or '00700'
        timeframe: '1D', '1W', '1M', '1m', '5m', etc.
        start_date: optional start date string 'YYYY-MM-DD'
        end_date: optional end date string 'YYYY-MM-DD'
        limit: max bars to return (capped at 1000)
    """
    # Quick pre-check: skip if FutuOpenD port is not reachable (avoids long timeout)
    _futu_host = os.environ.get("FUTU_HOST", "127.0.0.1")
    _futu_port = int(os.environ.get("FUTU_PORT", "11111"))
    if not _is_port_open(_futu_host, _futu_port, timeout=2.0):
        logger.debug(
            "FutuOpenD not reachable at %s:%s, skipping futu-api",
            _futu_host,
            _futu_port,
        )
        return []
    ...
    try:
        from futu import KLType, OpenQuoteContext  # type: ignore

        code = symbol.replace(".HK", "").replace(".hk", "").strip()
        if not code.startswith("HK."):
            code = f"HK.{code}"

        quote_ctx = OpenQuoteContext(host=_futu_host, port=_futu_port)
        ...
        ret, df, page_req_key = quote_ctx.request_history_kline(code, ktype=ktype, max_count=min(limit, 1000))
        quote_ctx.close()
...
# ---------------------------------------------------------------------------
# AKShare HK daily/weekly klines (Eastmoney — good quality, requires CN IP)
# ---------------------------------------------------------------------------


def fetch_akshare_hk_klines(
    symbol: str,
    timeframe: str = "1D",
    start: int | None = None,
    end: int | None = None,
    ...
    """
    Fetch HK stock daily/weekly klines via AKShare (Eastmoney backend).

    Args:
        symbol: HK stock code like 'HK.00700' or '00700'
        timeframe: '1D' or '1W'
        start: optional start timestamp (int, seconds)
        end: optional end timestamp (int, seconds)
        limit: max bars to return
    """
    try:
        import akshare as ak  # type: ignore
    except ImportError:
        logger.debug("akshare not installed; skipping AkShare HK klines")
        return []

    sym = ak_hk_code_from_tencent(symbol)
    if start:
        start_s = datetime.fromtimestamp(int(start)).strftime("%Y%m%d")
    else:
        start_s = (datetime.now() - timedelta(days=max(int(limit or 300), 1) * 2 + 400)).strftime("%Y%m%d")
    if end:
        end_s = datetime.fromtimestamp(int(end)).strftime("%Y%m%d")
    else:
    ...
    for attempt in range(_MAX_ATTEMPTS):
        try:
            ...
            if attempt + 1 < _MAX_ATTEMPTS and _is_transient(e):
                delay = min(_BACKOFF_CAP_SEC, _BACKOFF_BASE_SEC * (2**attempt))
                logger.debug(
                    "AkShare HK transient error sym=%s (attempt %s/%s): %s",
                    sym,
                    attempt + 1,
                    _MAX_ATTEMPTS,
                    e,
                ...
                time.sleep(delay)
                continue
            logger.warning("AkShare HK K-line failed sym=%s: %s", sym, e)
            return []
    ...
    for _, row in df.iterrows():
        try:
            ...
            continue
    out.sort(key=lambda x: x["time"])
    logger.debug("AkShare HK returned %d bars for sym=%s tf=%s", len(out), sym, timeframe)
    return out


def _ts_to_date_str(ts: int | None) -> str | None:
    """Convert Unix timestamp (int seconds) to YYYY-MM-DD string."""
    if ts is None:
        return None
