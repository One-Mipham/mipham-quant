"""
Mipham Quant — Flask application factory.
"""
    ...
    try:
        ...
        for strategy_info in running_strategies:
            ...
            try:
                ...
                logger.error(f"Error restoring strategy {strategy_id}: {e!s}")
                logger.error(traceback.format_exc())

        logger.info(f"Strategy restore completed: {restored_count}/{len(running_strategies)} restored")

    except Exception as e:
        logger.error(f"Failed to restore running strategies: {e!s}")
        logger.error(traceback.format_exc())
...
def create_app(config_name="default"):
    ...
    # CORS: restrict origins in production, allow local dev
    import os as _os

    _frontend_url = _os.getenv("FRONTEND_URL", "http://localhost:8888")
    _cors_origins = [_frontend_url]
    _extra = _os.getenv("OAUTH_ALLOWED_REDIRECTS", "")
    if _extra:
