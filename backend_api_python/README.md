# Mipham Quant Python API (backend)

Flask-based backend for Mipham Quant: market data, indicators, AI analysis, backtesting, and a strategy runtime with multi-user support.
...
CREATE DATABASE mipham;
CREATE USER mipham WITH ENCRYPTED PASSWORD 'your_password';
GRANT ALL PRIVILEGES ON DATABASE mipham TO mipham;
\q

# Initialize schema
psql -U mipham -d mipham -f migrations/init.sql
```
...
```bash
# Required
DATABASE_URL=postgresql://mipham:your_password@localhost:5432/mipham
...
`enabled=false` and the rest of Mipham Quant continues to work normally.
